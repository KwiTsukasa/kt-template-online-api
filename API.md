...
环境快照固定为七个稳定服务：Windows PC 的 `sunshine` 使用 Basic `GET /api/apps`、`codex-thread-index` 使用 `GET /readyz`；NAS 的 `nas-api` 使用进程健康检查、`home-assistant` 使用 Bearer `GET /api/` 且要求 `message="API running."`、`bot-core` 使用 QQBot 在线摘要；R4SE 的 `r4se-wireguard` 读取固定隧道地址、`r4se-mihomo` 使用 Bearer 读取 `version/configs/proxies`。外部服务缺配置为 `unwired`，401/403 或超时为脱敏 `degraded`，任何响应都不返回凭据或服务正文；共享 client 默认严格 TLS，只有固定 WireGuard Sunshine HTTPS 请求显式允许自签证书。
...
| `POST`   | `/system/network/port-forward-group/:groupId/channels/udp/natmap/enable`          | `super` | 启用 WireGuard UDP NATMap                 |
| `POST`   | `/system/network/port-forward-group/:groupId/channels/udp/natmap/disable`         | `super` | 停用 WireGuard UDP NATMap                 |
...
旧 `/port-forward` 路由保留平面通道兼容语义；多通道组必须使用逻辑组接口。新增和修改请求只接受各 DTO 白名单字段；普通通道目标 NAS IPv4 固定来自 `NETWORK_AGENT_TARGET_IPV4`，唯一 `51825 -> 51820/UDP` 端口对固定投影 R4SE `192.168.31.81`。未知字段返回 400。Snowflake ID 与 revision 在 HTTP JSON 中保留为字符串。所有动态响应设置 `Cache-Control: no-store`。
...
TCP current 必须同时满足 present、NATMap intent、synced、Router、互斥数据面证据、NATMap active、非空 generation、candidate/current/last-observed 同 tuple，以及接收时仍新鲜且未倒退的 lease；否则只保留候选/最近观测并撤下 current。WireGuard UDP NATMap 使用独立 `udp_natmap` mechanism，要求 NATMap intent、active generation 与三份同 tuple 新鲜租约，但不伪造 NAS Router、route-helper 或 DNAT 证据；普通 UDP Keeper 继续使用 `udp_stun`。事件按 `eventId` 幂等追加并保存真实 mechanism。
...
TCP 通道使用独立 NATMap 开关；普通 UDP 同端口通道使用 Keeper，固定 `51825 -> 51820` 的 WireGuard UDP 通道使用互斥 NATMap 开关。`NETWORK_TCP_NATMAP_RELEASE_MODE=off|draining|canary|on` 仅控制 TCP 变更范围。当前端点只有在 `currentValidUntil` 未过期时才返回为可用值；租约过期不会删除 candidate、last-observed、last-published 或 `network_endpoint_history`。API 不直接访问小米路由器、不修改 NAS 路由；真实进程、固定 UDP target 和回程证据由 Agent/helper/Companion 处理。

`GET /system/network/wireguard/endpoint` 只接受 `X-KT-Relay-Secret`，复用 `CODEX_REMOTE_PC_WS_SHARED_SECRET` 做定长比较，不接受 Admin JWT 替代。它仅在唯一 `51825 -> 192.168.31.81:51820` 通道达到 desired/reported revision 相等、`synced + active`、合法 endpoint identity 且租约未过期时返回 `{channelId,endpointIdentity,mechanism:'udp_natmap',publicIpv4,publicPort,reportedRevision,validUntil}`；其他状态统一为 `401/503` 且禁止缓存。
...
| Env Dashboard | `ENV_DASHBOARD_CACHE_TTL_MS`、`ENV_DASHBOARD_SIGNAL_TIMEOUT_MS`、`ENV_DASHBOARD_EVENT_BUS`、`ENV_DASHBOARD_MQTT_*`、`ENV_DASHBOARD_SSE_*`、`ENV_DASHBOARD_CODEX_THREAD_INDEX_URL`、`ENV_DASHBOARD_HOME_ASSISTANT_*`、`ENV_DASHBOARD_SUNSHINE_*`、`ENV_DASHBOARD_R4SE_*`                                                                                                                                                                                                                                                                    |
...
Env Dashboard 与 Mobile Home 共享精确三设备七服务拓扑。私有运行环境必须提供 Codex Thread Index、Home Assistant、Sunshine、R4SE WireGuard 与 Mihomo 所需键；缺失配置必须返回 `unwired`，Local Dev、Tencent Cloud、Caddy 及其旧键不会进入响应合同。
...
`POST /media-governance/series` 接受 TMDB TV 身份时会同时读取官方 `/tv/:id/seasons`
