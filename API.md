...
`GET /bot/permission/options` 返回全部未删除 Bot 账号，并按 `selfId → targetId` 级联目标与成员。NapCat 优先读取实时列表；官方账号或实时读取失败时返回已接收消息中的候选，响应中的 `source` 和 `notice` 标明来源。

群和频道的精确成员使用 `userIds: string[]`，一条名单包含同一会话的全部选中成员；更新覆盖该数组。同账号、同名单类型中已有该会话时应编辑原记录。旧 `userId` 单成员输入保持兼容。`view=tree` 返回完整筛选结果供账号分组；空 `selfId` 表示全局规则。

黑名单和白名单同时生效，先拒绝黑名单命中，再要求白名单命中；配置接口不再接受关闭任一名单。部署前需备份名单数据，镜像内迁移会补齐 `user_ids` JSON 列并保留旧成员字段；旧版本无法完整表达多成员规则，回滚需配套名单备份。
