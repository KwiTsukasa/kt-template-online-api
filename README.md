...
```text
src/
  apps/        独立进程入口（LLM Codex Gateway、NapCat WebUI Gateway）
  commands/    有界命令行入口
...
| Environment Dashboard | `ENV_DASHBOARD_CACHE_TTL_MS`、`ENV_DASHBOARD_SIGNAL_TIMEOUT_MS`、`ENV_DASHBOARD_EVENT_BUS`、`ENV_DASHBOARD_MQTT_*`、`ENV_DASHBOARD_SSE_*`、`ENV_DASHBOARD_CODEX_THREAD_INDEX_URL`、`ENV_DASHBOARD_HOME_ASSISTANT_*`、`ENV_DASHBOARD_SUNSHINE_*`、`ENV_DASHBOARD_R4SE_*`                                                                                                                                                                                          |
...
| Media Governance      | `MEDIA_GOVERNANCE_DESCRIPTOR_BUCKET`、`MEDIA_GOVERNANCE_EXECUTOR_BASE_URL`、`MEDIA_GOVERNANCE_EXECUTOR_INTERNAL_SECRET`、`MEDIA_GOVERNANCE_EXECUTOR_TIMEOUT_MS`；治理 Task 不绑定 Codex，NAS 刮削由独立模块记录                                                                                                                                                                                                                                                 |
...
Codex Remote 节点目录只返回固定 WireGuard 地址 `10.66.66.4:48095` 的 Windows
Desktop Relay。`GET /api/codex-remote/nodes` 返回唯一 Desktop writer 与项目，
`POST /api/codex-remote/nodes/:nodeId/session` 使用当前 Admin SSO 身份签发两分钟
HS256 Relay token；共享密钥永不返回客户端，NAS/PC 独立 `:48093` App Server 不再进入合同。
同一共享密钥还只保护 `GET /system/network/wireguard/endpoint` 的窄读取；该接口仅返回
已同步、未过期的 `51825 -> 192.168.31.81:51820` UDP NATMap tuple，不授予 Admin 权限。
...
Admin 环境总览与 Mobile Home 复用同一权威只读快照，固定为 `Windows PC -> Sunshine / Codex Thread Index`、`NAS -> API Runtime / Home Assistant / QQBot`、`R4SE -> WireGuard / Mihomo/OpenClash` 三个设备、七个服务。Local Dev、Tencent Cloud 与 Caddy 不再属于当前环境合同；HTTP 快照提供完整拓扑，后端 local/MQTT 事件总线通过 SSE 推送增量事件，所有公开事件都以必填 `eventId` 作为 SSE 与客户端去重身份，不再混用 `id`。客户端不直连 MQTT，也不轮询刷新。任一外部探针缺少配置时必须返回 `unwired`，不能渲染成健康假象；接口不暴露服务控制或代理切换写入口。
...
智能家居首批能力通过 `GET /system/mobile-home/home` 聚合 Home Assistant area/device/entity registry、states、24 小时 logbook 与能源历史；常见英文区域和 HA 内置备份/太阳历实体按稳定 ID 投影为中文，用户自定义名称保持原文，原始 entityId/state 继续用于动作和诊断。`POST /system/mobile-home/home/service` 只允许开关、场景和有限气候参数并用 `requestId` 幂等，`POST /system/mobile-home/home/assist` 只投影 Conversation 的 speech、类型和会话 ID。游戏首批能力通过 `GET /system/mobile-home/game` 返回 Sunshine 管理目录、固定 WireGuard 主机、GameStream 端口族、ViGEm 兼容结论、显示分辨率及虚拟显示就绪布尔；分辨率优先取 `/api/logs` 最近实际捕获，冷启动尚无捕获时从最新结构化显示设备清单取 `/api/config.output_name` 对应当前模式；`virtualDisplayReady` 还要求 `ensure_active/auto`。Android 每次启动按手机尺寸请求 Sunshine 串流期虚拟显示，物理主屏始终保持活动、主屏身份与原分辨率。`POST /system/mobile-home/game/pin` 只替内嵌 Moonlight 客户端提交四位临时 PIN。所有接口继续要求 Admin JWT + `super`、`Cache-Control: no-store`，不返回 HA token、Sunshine Basic、原始 attributes/context、运行日志、本地图片路径或证书正文。

Environment Dashboard 的外部连接只使用固定只读 API：Codex Thread Index 仅 `GET /readyz`；Home Assistant 以 Bearer `GET /api/` 并要求精确健康消息；Sunshine 以 Basic `GET /api/apps` 且不投影应用清单；R4SE WireGuard 只读取固定隧道地址；Mihomo 以 Bearer 读取 `version/configs/proxies`。缺任一配置时返回对应稳定服务的 `unwired`，认证失败、非成功 HTTP 状态与超时只返回稳定中文脱敏摘要。URL、token、用户名、密码与 Mihomo Secret 只存在于私有运行环境；共享 HTTP client 默认严格 TLS，只有固定 WireGuard Sunshine HTTPS adapter 显式允许其自签证书。

System 网络管理以 MySQL 中的逻辑端口转发组和 TCP/UDP 协议通道为唯一事实源。`super` 可在同一外部端口配置 `TCP`、`UDP` 或 `TCP+UDP`，并分别启停 TCP NATMap、固定 WireGuard UDP NATMap 与普通 UDP Keeper；UDP NATMap 只接受 `51825 -> 51820`，并把目标固定到 R4SE `192.168.31.81`。TCP 发布范围由 `NETWORK_TCP_NATMAP_RELEASE_MODE` 和 canary 端口控制。Agent 状态接口返回当前生效的 `tcpReleaseMode`；NATMap 启停接口可选携带 `expectedDesiredRevision`，并在事务锁内、no-op 判断前执行 CAS。API 在事务内单调提升 revision，提交后按当前 schema owner 使用 `kt/network/v1|v2/agents/{agentId}`、QoS 1 retained 完整快照通知 NAS `kt-network-agent`，自身不登录路由器、不接收路由器密码，也不执行 raw socket。Agent 声明 v2 capability 且发布门禁允许后，v2 成为唯一 desired/reported/events 写入者，迟到的 v1 reported/events 不再覆盖状态。

MQTT v2 分别保存 candidate、current、last-observed、last-published、通道 report 原始 RFC3339Nano 水位，以及静态路径、Keeper 和 NATMap 错误。TCP current 继续要求 Router、互斥数据面、NATMap generation 与三份同 tuple 新鲜租约；WireGuard UDP NATMap 则要求独立 `udp_natmap` intent、generation、candidate/current/last-observed 与租约，但明确不要求或伪造 NAS Router/route-helper/DNAT。普通 UDP Keeper 仍使用 `udp_stun`。相同 tuple 续租只推进 validation/lease，不新增历史、不触发 DDNS、Outbox 或 Admin 刷新。同 revision 的旧 report 不得覆盖较新的通道状态；合法外层 snapshot 完成协调尝试后推进 applied revision 与独立 applied schema 水位，单通道 conflict 只隔离该通道。Agent 接受 v2 后会耐久锁存唯一 owner，在未来实现 Agent 可确认的降级握手前，API 单边降级固定拒绝。
...
Admin 媒体治理生产链路使用 `JwtAuthGuard` 与媒体专用权限门，并严格收口为
`固化身份 → 绑定来源/下载 → 目录与文件名归一化 → 机械验收关闭`。作品目录继续采用
`Series → Work → Season/Episode → Task`：Series 创建同时建立主 Work，TV Work
可以创建 Season/Episode，电影和剧场版保持无伪季的独立 Work；Task 只能绑定既有 Work
和已确认 Episode，不能从下载标题反推或新建作品身份。

治理执行合同只包含来源检查、探针、下载、治理事务和 `acceptance.verify`。
`governance.execute` 成功后自动预约机械验收，验收通过即以
`closedMode=mechanical` 关闭。Task 不再拥有 `metadataStatus`、A/B/C 缺项、
媒体 Agent session、LLM conversation 或人工决策状态，也不再提供
`metadata/verify`、`metadata/repair`、`agent/start` 等入口。迁移前停在
`metadata` 的历史 Task 会在启动时迁移到机械验收；缺少密封计划时回到可见的治理
阻断态，绝不会因为 NAS 刮削缺项被要求进入本地 Codex。

NFO、海报、资料库关联与 trim.media 可见性属于独立
`media-scrape-validation` 模块。机械 Task 关闭后只异步登记身份与验收快照；
独立记录使用 `pending/running/healthy/issues` 状态，NAS 执行器通过内部 claim/result
接口领取和回传结果。刮削登记失败、校验缺项或重试都不回写 Task 的
`stage/runState/gateReason`，不阻止规范文件进入 NAS 或直接播放。Admin 在
`/media/scrape-validation` 提供单独页面，治理 Task 详情不再显示元数据页签。

多来源下载、逐文件 Unit/角色/季集映射、字幕发布组合同、来源健康探针、下载续传、
电影规范版本原子替换、同设备 hardlink 回滚、Redis 高频进度与 MySQL 终态事务仍保留。
电影升级只有候选机械验收成功时才在同一事务退役旧 Task；判断不依赖文件大小，也不依赖
NAS 刮削状态。Series/Work 自动同步只读取已机械关闭 Task 的固化身份和 Episode 映射，
不再检查刮削状态。

以 TMDB TV 身份新建 Series 时，API 会在写事务前重新读取官方季列表，并在同一事务建立
...
`migrate-media-governance-series-work` K8s initContainer 在数据库备份后的标准发布窗口执行，
用于补建独立刮削记录并移除旧媒体 Agent/LLM/元数据投影表列；配套 verify SQL 会阻止缺表、
遗留列、孤立校验记录或已关闭 Task 漏回填的版本启动。

NAS executor 所需的 manifest、事务备份、目录合同、字幕、回滚与 trim 官方 helper
统一由 API 项目的 `scripts/media-governance/runtime/` 持有并测试。
根目录 `ci/media-governance-executor/build-release.sh` 只从该 API 路径构建发布包；
`mcp/ktWorkflow` 不再包含 `kt_media_*`、媒体能力档、ledger、media/trim 脚本
或专用测试。普通 Admin LLM/Codex 对话继续属于通用 LLM 模块，与媒体治理 Task 无绑定关系。
