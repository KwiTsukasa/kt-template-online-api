...
| Environment Dashboard | `ENV_DASHBOARD_CACHE_TTL_MS`、`ENV_DASHBOARD_SIGNAL_TIMEOUT_MS`、`ENV_DASHBOARD_EVENT_BUS`、`ENV_DASHBOARD_MQTT_*`、`ENV_DASHBOARD_SSE_*`、`ENV_DASHBOARD_CODEX_APP_SERVER_URL`、`ENV_DASHBOARD_HOME_ASSISTANT_*`、`ENV_DASHBOARD_SUNSHINE_*`、`ENV_DASHBOARD_R4SE_*`                                                                                                                                                                                           |
...
Environment Dashboard 的外部连接只使用固定只读 API：Codex App Server 仅 `GET /readyz`；Home Assistant 以 Bearer `GET /api/` 并要求精确健康消息；Sunshine 以 Basic `GET /api/apps` 且不投影应用清单；R4SE WireGuard 只读取固定隧道地址；Mihomo 以 Bearer 读取 `version/configs/proxies`。缺任一配置时返回对应稳定服务的 `unwired`，认证失败与超时只返回脱敏 `degraded`。URL、token、用户名、密码与 Mihomo Secret 只存在于私有运行环境；共享 HTTP client 默认严格 TLS，只有固定 WireGuard Sunshine HTTPS adapter 显式允许其自签证书。
