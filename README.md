...
公开 `open.kwitsukasa.top/<service>/` 仅通过腾讯云 Caddy 与 WireGuard 调用 Host 限定的 `/network/open-redirect/:serviceKey`。API 不接收任意重定向目标，而是从固定服务白名单选取 Host/路径，并在同一事务中确认 `10443/TCP` NATMap 通道、逻辑组、`nas4` A 记录和 Agent 都处于当前且已同步状态后返回临时 `302`；未知服务为 `404`，离线、lease 过期、revision 落后、DDNS 不一致或数据库异常为无 `Location` 的 `503`。首跳后浏览器直接访问 `https://*.nas4.kwitsukasa.top:{动态端口}/...`，业务数据不经过腾讯云。
