...
export type PluginCliCommand = 'create' | 'install-local' | 'pack' | 'validate';
...
const packageExtension = '.plugin.json';
...
const getControlledRoots = (options: Required<PluginCliOptions>) => {
...
const resolveControlledPath = (
  pathArg: string,
  options: Required<PluginCliOptions>,
  label: string,
) => {
...
function assertPluginKey(
  ...
  if (!pluginKey) throw new Error('Usage: plugin create <pluginKey>');
...
const readManifest = (pluginRoot: string) => {
  ...
  return parsePluginManifest(manifest, { pluginRoot });
};
...
const assertPackageIntegrity = (packedPlugin: PackedPlugin) => {
  ...
  if (packedPlugin.contentHash !== expectedHash) {
    throw new Error('Plugin package content hash mismatch.');
  }

  parsePluginManifest(packedPlugin.manifest);
};
...
const createPlugin = (
  ...
  options.stdout(`Created plugin: ${pluginRoot}`);
  return {
    command: 'create',
    exitCode: 0,
    pluginKey,
...
const validatePlugin = (
  ...
  if (!pluginRootArg) throw new Error('Usage: plugin validate <path>');

  const pluginRoot = path.resolve(options.cwd, pluginRootArg);
  const manifest = readManifest(pluginRoot);
  assertPluginSourceBoundary(pluginRoot);
  options.stdout(`Validated plugin: ${manifest.pluginKey}@${manifest.version}`);
...
const packPlugin = (
  ...
  if (!pluginRootArg) throw new Error('Usage: plugin pack <path>');
  ...
  options.stdout(`Packed plugin: ${packagePath}`);
...
const installLocalPlugin = (
  ...
  if (!packagePathArg) {
    throw new Error('Usage: plugin install-local <package>');
  }
  ...
  options.stdout(
    `Installed local plugin package: ${packedPlugin.manifest.pluginKey}@${packedPlugin.manifest.version}`,
  );
...
export const runPluginCli = async (
  ...
  const command = argv[0] as PluginCliCommand | undefined;
  const resolvedOptions: Required<PluginCliOptions> = {
  ...
  switch (command) {
    ...
    default:
      throw new Error(
        'Usage: plugin <create|validate|pack|install-local> [args]',
      );
  }
};
