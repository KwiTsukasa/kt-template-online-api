-- 消息管理协议内核与 Bot、站内信订阅者生产增量初始化。
-- 通用模板、订阅、事件归消息管理；Bot Adapter 与站内信仅保留各自投递配置。
...
-- 兼容上一版 Bot 私有消息表：先复制协议数据，再移除订阅者私有模板列。
SET @legacy_message_template_exists := (
  SELECT COUNT(*)
  FROM information_schema.tables
  WHERE table_schema = DATABASE()
    AND table_name = 'bot_message_template'
);
...
SET @legacy_message_event_exists := (
  ...
  FROM information_schema.tables
  WHERE table_schema = DATABASE()
    AND table_name = 'bot_message_event'
);
SET @migration_sql := IF(
...
SET @legacy_subscription_ready := (
  SELECT
    (SELECT COUNT(*) FROM information_schema.tables
     WHERE table_schema = DATABASE() AND table_name = 'bot_message_subscription')
    +
...
-- 旧模型把模板放在 Bot 账号绑定上。若同一旧订阅被不同账号选了不同模板，
  ...
  'INSERT INTO legacy_message_subscription_template_map (
   ...
   FROM (
     ...
     FROM (
       ...
       FROM bot_message_publish_binding
       WHERE is_deleted = 0
...
SET @migration_sql := IF(
  ...
  'INSERT INTO message_subscription (
   ...
   SELECT
     ...
     legacy.create_time,
     legacy.update_time
   FROM bot_message_subscription legacy
...
UPDATE bot_message_delivery delivery
JOIN legacy_message_subscription_template_map mapping
...
SET @migration_sql := IF(
  @legacy_subscription_ready = 2,
  'UPDATE bot_message_publish_binding binding
   JOIN legacy_message_subscription_template_map mapping
...
SET @legacy_delivery_index_exists := (
  ...
  WHERE table_schema = DATABASE()
    ...
    AND index_name = 'uk_bot_message_delivery_event_target'
);
SET @migration_sql := IF(
  @legacy_delivery_index_exists > 0,
  'ALTER TABLE bot_message_delivery DROP INDEX uk_bot_message_delivery_event_target',
  'SELECT 1'
);
PREPARE message_migration FROM @migration_sql;
EXECUTE message_migration;
...
SET @message_delivery_template_index_exists := (
  ...
  WHERE table_schema = DATABASE()
    AND table_name = 'bot_message_delivery'
    AND index_name = 'uk_bot_message_delivery_event_target_template'
);
SET @migration_sql := IF(
  @message_delivery_template_index_exists = 0,
  'ALTER TABLE bot_message_delivery
   ADD UNIQUE KEY uk_bot_message_delivery_event_target_template
