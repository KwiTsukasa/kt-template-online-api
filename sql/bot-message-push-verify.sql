...
WITH required_index AS (
  ...
  UNION ALL SELECT 'bot_message_publish_binding', 'PRIMARY', 0, 1, 'id'
  UNION ALL SELECT 'bot_message_publish_binding', 'uk_bot_message_publish_binding_active_key', 0, 1, 'active_key'
  UNION ALL SELECT 'bot_message_publish_target', 'PRIMARY', 0, 1, 'id'
  UNION ALL SELECT 'bot_message_publish_target', 'uk_bot_message_publish_target_active_key', 0, 1, 'active_key'
  UNION ALL SELECT 'bot_message_delivery', 'PRIMARY', 0, 1, 'id'
  UNION ALL SELECT 'bot_message_delivery', 'uk_bot_message_delivery_event_target_template', 0, 1, 'message_event_id'
  UNION ALL SELECT 'bot_message_delivery', 'uk_bot_message_delivery_event_target_template', 0, 2, 'publish_target_id'
  UNION ALL SELECT 'bot_message_delivery', 'uk_bot_message_delivery_event_target_template', 0, 3, 'template_id'
  UNION ALL SELECT 'bot_message_delivery', 'idx_bot_message_delivery_dispatch', 1, 1, 'status'
  UNION ALL SELECT 'bot_message_delivery', 'idx_bot_message_delivery_dispatch', 1, 2, 'next_attempt_at'
  UNION ALL SELECT 'bot_message_delivery', 'idx_bot_message_delivery_lease', 1, 1, 'processing_lease_until'
  UNION ALL SELECT 'bot_message_delivery', 'idx_bot_message_delivery_history', 1, 1, 'subscription_id'
  UNION ALL SELECT 'bot_message_delivery', 'idx_bot_message_delivery_history', 1, 2, 'message_event_id'
...
SELECT 'bot_binding_forbidden_template_column' AS check_name, COUNT(*) AS invalid_rows
FROM information_schema.columns
WHERE table_schema = DATABASE()
...
SELECT 'bot_delivery_duplicate_event_target_template' AS check_name, COUNT(*) AS invalid_rows
FROM (
