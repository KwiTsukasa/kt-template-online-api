...
export async function persistMigratedDefinition(
  ...
  table: 'automation_task' | 'automation_trigger' | 'automation_schedule',
  input: {
    sourceKey: string;
    name: string;
    description: string;
...
/**
 * 迁移为三个所属模块的独立资源，旧任务身份只用于任务资源，计划通过固定引用关联。
...
export async function migrateResourceSet(
  ...
  const definition: ScheduleDefinition = {
    ...
    target: { type: 'task', reference: { id: task.id, version: task.version } },
    input: input.input,
    admission: null,
    overlap: 'skip',
