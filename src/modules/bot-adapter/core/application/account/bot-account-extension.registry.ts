...
export interface BotAccountExtension {
...
@Injectable()
export class BotAccountExtensionRegistry {
  private readonly extensions = new Map<string, BotAccountExtension>();

  /**
   * 注册一个 Bot 账号扩展，并拒绝重复键以保证账号清理副作用唯一。
   * @param extension - 需要参与账号生命周期的外部扩展。
   * @throws 同一扩展键已有实例时抛出重复注册错误。
   */
  register(extension: BotAccountExtension): void {
    if (this.extensions.has(extension.extensionKey)) {
      throw new Error(
        `Duplicate Bot account extension: ${extension.extensionKey}`,
      );
    }
