...
import {
  ...
  type BotAccountNapcatRuntimePort,
} from './bot-account-napcat-runtime.port';
import { BotAccountAbility } from '../../infrastructure/persistence/account/bot-account-ability.entity';
import { BotAccount } from '../../infrastructure/persistence/account/bot-account.entity';
import { BotAccountExtensionRegistry } from './bot-account-extension.registry';
import type {
  BotAccountBodyDto,
  BotAccountQueryDto,
  BotAccountUpdateDto,
} from '../../contract/account/bot-account.dto';
import {
  BOT_DEFAULT_PAGE_NO,
  BOT_DEFAULT_PAGE_SIZE,
} from '../../contract/bot.constants';
import type {
  BotAccountAbilityType,
  BotAccountListItem,
  BotConnectionMode,
  BotConnectionRole,
  NapcatRuntimeLoginStatus,
  NapcatWebuiStatus,
  BotRuntimeContainerStatus,
...
const TENCENT_BOT_APP_ID_PATTERN = /^\d{5,20}$/;
const TENCENT_BOT_CONNECTION_MODES: BotConnectionMode[] = [
  'official-webhook',
  'official-websocket',
];
const BOT_CONNECTION_MODES: BotConnectionMode[] = [
  'reverse-ws',
  ...TENCENT_BOT_CONNECTION_MODES,
];
const TENCENT_BOT_SELF_ID_PREFIX = 'qq-official:';
...
export class BotAccountService {
  constructor(
    ...
    @InjectRepository(BotAccountAbility)
    private readonly accountAbilityRepository: Repository<BotAccountAbility>,
    private readonly toolsService: ToolsService,
    @Optional()
    @Inject(BOT_ACCOUNT_NAPCAT_RUNTIME_PORT)
    private readonly napcatRuntime?: BotAccountNapcatRuntimePort,
    @Optional()
   ...
   * @param connectionModes - 可选适配器连接模式集合；提供时只返回对应 BotAdapter 账号。
   * @returns 包含 `list`、`pageNo`、`pageSize`、`total` 字段的Bot 管理分页结果。
   */
  async page(query: BotAccountQueryDto, connectionModes?: BotConnectionMode[]) {
    ...
    if (connectionModes && connectionModes.length > 0) {
      builder.andWhere('account.connectionMode IN (:...connectionModes)', {
        connectionModes,
      });
    }
  ...
  /**
   * 读取全部启用账号，并可按 BotAdapter 连接模式过滤。
   * @param connectionModes - 可选适配器连接模式集合。
   * @returns 按创建时间排列的启用账号。
   */
  async allEnabled(connectionModes?: BotConnectionMode[]) {
    const where: Record<string, unknown> = {
      enabled: true,
      isDeleted: false,
    };
    if (connectionModes && connectionModes.length > 0) {
      where.connectionMode = In(connectionModes);
    }
  ...
  async allEnabledOfficialWithSecret() {
    return this.accountRepository
      ...
      .where('account.connectionMode IN (:...connectionModes)', {
        connectionModes: TENCENT_BOT_CONNECTION_MODES,
      })
  ...
  async getDefaultAccount(selfId?: string) {
    ...
    if (accounts.length > 1) {
      throwVbenError('存在多个可用 Bot 账号，请明确选择发送账号');
    }
   ...
   * @param selfId - 官方 Bot AppID，同时也是现有 Bot 账号稳定键。
  ...
  async ensureScannedAccount(input: {
    ...
    const payload: Partial<BotAccount> = {
      accessToken: null,
      clientRole: null,
   ...
   * @returns `save` 对应。
   */
  async save(body: BotAccountBodyDto) {
  ...
  /**
   * 根据`body`更新Bot账号记录；先通过 `assertSelfIdAvailable` 校验输入边界。
   * @param body - 用于Bot账号记录的结构化输入，包含 `selfId`、`id`、`accessToken` 字段。
   * @returns 满足Bot账号记录约束时为 `true`；不满足、未命中或显式失败分支为 `false`。
   */
  async update(body: BotAccountUpdateDto) {
    ...
    await this.accountRepository.manager.transaction(async (manager) => {
      const accounts = manager.getRepository(BotAccount);
      ...
      if (selfIdChanged) {
        await manager
          .getRepository(BotAccountAbility)
  ...
  /**
   * 按`id`移除Bot账号记录；从 `accountRepository.findOne` 读取Bot账号记录。
   * @param id - 决定Bot账号记录内容、边界或目标的 `id` 值。
   * @returns 包含 `deletedContainers` 字段的Bot账号记录。
   */
  ...
  async markOfficialOnline(selfId: string) {
    await this.accountRepository.update(
      { connectionMode: In(TENCENT_BOT_CONNECTION_MODES), selfId },
      {
        clientRole: null,
        connectStatus: 'online',
        lastConnectedAt: new Date(),
  ...
  async markOfficialOffline(selfId: string, lastError?: string) {
    const payload: Partial<BotAccount> = { connectStatus: 'offline' };
    if (lastError !== undefined) {
      if (lastError) {
        payload.lastError = this.toolsService.toColumnText(lastError, 500);
      } else {
  ...
  getNapcatLoginPassword(
    account?: Pick<BotAccount, 'napcatLoginPasswordSecret'> | null,
  ) {
  ...
  getOfficialAppSecret(
    account?: Pick<BotAccount, 'officialAppSecretCiphertext'> | null,
  ) {
   ...
   */
  private async syncPersistedNapcatSplitStatus(account: BotAccountListItem) {
   ...
   */
  private hasPersistedNapcatSplitStatus(account: BotAccountListItem) {
    return [
      'oneBotStatus',
      'containerStatus',
      'webuiStatus',
  ...
  private toPersistedContainerStatus(
    napcat?: null | BotAccountListItem['napcat'],
  ): BotRuntimeContainerStatus {
  ...
  private async assertSelfIdAvailable(selfId: string, id?: string) {
    ...
    if (exists && exists.id !== id) {
      throwVbenError(
        (() => {
          if (exists.isDeleted) {
            return 'Bot 账号 selfId 已存在于已删除账号，请通过新增恢复该账号';
          }
          return 'Bot 账号 selfId 已存在';
        })(),
      );
    }
  }
   ...
   */
  private async restoreDeletedAccount(payload: Partial<BotAccount>) {
   ...
   * @param body - Bot 账号创建或局部编辑请求。
   * @param create - 是否应用新账号的反向 WS、启用、名称和备注默认值。
   * @returns 仅包含当前请求允许更新字段的账号载荷。
   */
  private normalizeBody(body: Partial<BotAccountBodyDto>, create: boolean) {
    const payload: Partial<BotAccount> = {};
    if (
      body.connectionMode &&
      !BOT_CONNECTION_MODES.includes(body.connectionMode)
    ) {
      throwVbenError('Bot 接入方式无效');
    }
    ...
    if (
      ...
      if (appId) {
        if (!TENCENT_BOT_APP_ID_PATTERN.test(appId)) {
   ...
   */
  private assertCreateCredentials(payload: Partial<BotAccount>) {
  ...
  private applyCredentialBoundary(
    payload: Partial<BotAccount>,
    current: BotAccount,
    body: Partial<BotAccountBodyDto>,
  ) {
  ...
  private officialSelfId(appId: string) {
    return `${TENCENT_BOT_SELF_ID_PREFIX}${appId}`;
  }
  ...
  private isOfficialConnectionMode(
    ...
    return TENCENT_BOT_CONNECTION_MODES.includes(
      connectionMode as BotConnectionMode,
    );
  }
  ...
  private getAccountSecretKey() {
    const secret = this.toolsService.pickFirstText(
      this.configService?.get<string>('BOT_ACCOUNT_SECRET_KEY'),
      this.configService?.get<string>('ADMIN_TOKEN_SECRET'),
    );
    if (!secret || INSECURE_ACCOUNT_SECRET_VALUES.has(secret)) {
      throwVbenError(
        'Bot 账号登录密码密钥未配置，请设置 BOT_ACCOUNT_SECRET_KEY 或 ADMIN_TOKEN_SECRET',
      );
    }
    return secret;
  }
  ...
  private async assertConfigurableAccount(selfId: string) {
    const normalizedSelfId = `${selfId || ''}`.trim();
    if (!normalizedSelfId) throwVbenError('请选择所属 Bot 账号');
    const account = await this.findBySelfId(normalizedSelfId);
    if (!account || !account.enabled) {
      throwVbenError(`Bot 账号不存在或已停用：${normalizedSelfId}`);
    }
    return account;
  }
