...
import type { BotCommandMatchResult } from '../../contract/bot.types';
...
export class BotCommandParserService {
   ...
   */
  async match(command: BotCommand, message: BotNormalizedMessage) {
   ...
   * @returns 别名集合。
   */
  async getAliases(command: BotCommand) {
   ...
   * @returns 命令前缀集合。
   */
  getPrefixes(command: BotCommand) {
   ...
   */
  private async getManifestAliases(command: BotCommand) {
