...
import type {
  ...
  BotCommandQueryDto,
  BotCommandUpdateDto,
} from '../../contract/command/bot-command.dto';
import { isWithinCooldown } from '../../domain/bot-cooldown.policy';
import { BotCommandLog } from '../../infrastructure/persistence/command/bot-command-log.entity';
import { BotCommand } from '../../infrastructure/persistence/command/bot-command.entity';
...
export class BotCommandService {
  constructor(
    ...
    @InjectRepository(BotCommandLog)
    private readonly commandLogRepository: Repository<BotCommandLog>,
   ...
   */
  async page(query: BotCommandQueryDto) {
   ...
   * @param adapterContext - 当前 transport 已授权的插件键；提供后只返回这些插件拥有的命令。
   * @returns 启用状态消息。
   */
  async listEnabledForMessage(
    ...
    if (adapterContext) {
      const pluginKeys = [...new Set(adapterContext.pluginKeys)];
      if (pluginKeys.length === 0) return [];
      builder.andWhere('command.pluginKey IN (:...pluginKeys)', {
        pluginKeys,
      });
    } else {
      const boundIds = await this.accountService.getBoundCommandIds(
   ...
   * @returns `save` 对应。
   */
  async save(body: BotCommandBodyDto) {
   ...
   */
  async update(body: BotCommandUpdateDto) {
   ...
   */
  async markHit(command: BotCommand) {
   ...
   */
  isInCooldown(command: BotCommand) {
   ...
   * @returns 参数。
   */
  parseDefaultParams(command: BotCommand) {
   ...
   */
  private async normalizeBody(body: BotCommandBodyDto) {
    ...
    return {
      ...
      name: body.name.trim(),
      operationKey,
      parserKey: (body.parserKey || 'plain') as BotCommandParserType,
      pluginKey,
   ...
   */
  private toRawBody(command: BotCommand): BotCommandBodyDto {
    return {
      aliases: this.parseList(command.aliases),
      code: command.code,
      cooldownMs: command.cooldownMs,
  ...
  private getMinCooldownMs() {
    const value = Number(this.configService.get('BOT_COMMAND_MIN_COOLDOWN_MS'));
