...
import type {
  ...
  BotMessageQueryDto,
} from '../../contract/message/bot-message.dto';
import type {
  BotMessageType,
...
export class BotMessageService {
   ...
   */
  async conversationPage(query: BotConversationQueryDto) {
   ...
   */
  async messagePage(query: BotMessageQueryDto) {
    const { pageNo, pageSize, skip } = this.toolsService.getPageParams(
      query,
      BOT_DEFAULT_PAGE_NO,
      BOT_DEFAULT_PAGE_SIZE,
   ...
   * @returns 入站消息。
   */
  async saveIncoming(message: BotNormalizedMessage) {
   ...
   */
  private async upsertConversation(message: BotNormalizedMessage) {
