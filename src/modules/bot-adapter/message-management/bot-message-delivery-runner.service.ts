...
} from '@/modules/bot-adapter/core/application/send/bot-send.error';
...
type PreparedDelivery =
  | { kind: 'send'; delivery: BotMessageDelivery }
  | { kind: 'stale' }
  | {
      code: string;
      kind: 'finish';
...
export class SystemMessageDeliveryRunnerService {
  ...
  private async claimOne(now: Date): Promise<ClaimToken | null> {
    return this.dataSource.transaction(async (manager) => {
      const deliveries = manager.getRepository(BotMessageDelivery);
  ...
  private async processClaim(
    ...
    try {
      ...
      if (error instanceof BotSendAttemptError) {
        if (!error.retryable) {
          await this.finish(
            token,
            'failed',
  ...
  private async prepare(token: ClaimToken): Promise<PreparedDelivery> {
    return this.dataSource.transaction(async (manager) => {
      ...
      const account = await (async () => {
        if (binding) {
          return await manager.getRepository(BotAccount).findOne({
      ...
      const delivery = await manager
        .getRepository(BotMessageDelivery)
        .findOne({
   ...
   */
  private validateFrozen(delivery: BotMessageDelivery): void {
  ...
  private async persistOwnerTransition(
    ...
    const deliveries = this.dataSource.getRepository(BotMessageDelivery);
   ...
   */
  private owns(delivery: BotMessageDelivery, token: ClaimToken): boolean {
    return (
      delivery.status === 'processing' &&
