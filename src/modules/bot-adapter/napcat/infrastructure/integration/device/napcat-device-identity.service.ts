...
export class NapcatDeviceIdentityService {
   ...
   * @returns 物理的OUIMAC地址。
   */
  private buildPhysicalOuiMacAddress(accountId: string, containerName: string) {
    const hash = createHash('sha256')
      .update(`${accountId}:${containerName}:${QQNT_PHYSICAL_OUI_MAC_STRATEGY}`)
      .digest('hex');
    const prefix =
      NAPCAT_PHYSICAL_OUI_PREFIXES[
        parseInt(hash.slice(0, 8), 16) % NAPCAT_PHYSICAL_OUI_PREFIXES.length
      ];
    const suffix = [hash.slice(8, 10), hash.slice(10, 12), hash.slice(12, 14)];
