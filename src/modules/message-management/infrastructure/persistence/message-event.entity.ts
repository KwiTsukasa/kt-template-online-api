...
import type {
  SystemMessageFanoutStatus,
  SystemMessageScalar,
} from '../../contract/message-management.types';

@Entity('message_event')
@Index('uk_message_event_event_id', ['eventId'], { unique: true })
@Index('idx_message_event_dispatch', ['fanoutStatus', 'nextFanoutAt'])
@Index('idx_message_event_lease', ['fanoutLeaseUntil'])
@Index('idx_message_event_source_resource_order', [
  'sourceKey',
  'resourceKey',
  'occurredAt',
  'id',
...
export class MessageEvent {
  ...
  @KtDateTimeColumn({ name: 'occurred_at', precision: 6 })
  occurredAt: KtDateTime;
  @Column({ type: 'json' }) payload: Record<string, SystemMessageScalar>;
  @Column({ default: 'accepted', length: 32, name: 'fanout_status' })
  fanoutStatus: SystemMessageFanoutStatus;
  @Column({
    default: 0,
    name: 'fanout_attempt_count',
    type: 'int',
    unsigned: true,
  })
  fanoutAttemptCount: number;
  @KtDateTimeColumn({ name: 'next_fanout_at', nullable: true, precision: 6 })
  nextFanoutAt: KtDateTime | null;
  @KtDateTimeColumn({
    name: 'fanout_lease_until',
    nullable: true,
    precision: 6,
  })
  fanoutLeaseUntil: KtDateTime | null;
