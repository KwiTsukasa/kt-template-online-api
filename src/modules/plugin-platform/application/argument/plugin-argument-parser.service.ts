...
import {
  ...
  type PluginInputNormalizerPort,
} from './plugin-input-normalizer.port';

@Injectable()
export class PluginArgumentParserService {
  constructor(
    @Optional()
    @Inject(PLUGIN_INPUT_NORMALIZER)
    private readonly normalizer?: PluginInputNormalizerPort,
  ) {}
   ...
   */
  async normalizeInput(input: BotPluginOperationInput) {
