...
import {
  PluginTask,
  PluginTaskRun,
  type PluginTaskRunStatus,
  type PluginTaskTriggerType,
} from '../../infrastructure/persistence';
import {
  PLUGIN_TASK_JOB_NAME,
  PLUGIN_TASK_QUEUE_NAME,
  readPluginTaskQueuePrefix,
  resolvePluginTaskQueueConnection,
  resolveNextPluginTaskRunAt,
  type PluginTaskJobData,
} from './plugin-task-scheduler.service';

@Injectable()
export class PluginTaskWorkerProcessor
  implements OnModuleDestroy, OnModuleInit
{
  private readonly logger = new Logger(PluginTaskWorkerProcessor.name);
  private worker?: Worker<PluginTaskJobData>;

  constructor(
    private readonly configService: ConfigService,
    private readonly platformService: PluginPlatformService,
  ...
  async onModuleInit() {
    this.worker = new Worker<PluginTaskJobData>(
      PLUGIN_TASK_QUEUE_NAME,
      async (job) => this.processJob(job),
      {
        concurrency: 1,
        connection: resolvePluginTaskQueueConnection(this.configService),
        prefix: readPluginTaskQueuePrefix(this.configService),
      },
    );
  ...
  /**
   * 根据`job`处理子进程Job；当 `job.name && job.name !== PLUGIN_TASK_JOB_NAME` 成立时返回 `{ ok: false, reason: 'unknown-job', skipped…`。
   * @param job - 用于子进程Job的领域对象，包含 `name`、`data`、`id` 字段。
   * @returns 子进程Job。
   */
  private async processJob(job: Job<PluginTaskJobData>) {
    if (job.name && job.name !== PLUGIN_TASK_JOB_NAME) {
   ...
   */
  private resolveNextRunAt(task: PluginTask) {
    if (!task.enabled || !task.cronExpression) return null;
    return resolveNextPluginTaskRunAt(task.cronExpression);
  }
