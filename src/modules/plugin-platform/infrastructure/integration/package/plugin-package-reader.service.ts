...
import {
  parsePluginManifest,
  type PluginManifest,
} from '@/modules/plugin-platform/domain/manifest';
...
const PACKAGE_EXTENSION = '.plugin.json';
...
@Injectable()
export class PluginPackageReaderService {
  constructor(
    @Optional()
   ...
   */
  readPackage(body: PluginPackageBody): ValidatedPluginPackage {
    ...
    try {
      ...
      throwVbenError('Bot 插件包不是合法 JSON', undefined, error);
    }
    ...
    if (contentHash !== expectedHash) {
      throwVbenError('Bot 插件包 hash 校验失败');
    }
    if (body.packageHash && body.packageHash !== expectedHash) {
      throwVbenError('Bot 插件包 hash 与请求不一致');
    }

    return {
      manifest: parsePluginManifest(packedPlugin.manifest, {
  ...
  private getControlledRoots() {
    const configuredRoots = [
      this.configService?.get<string>('PLUGIN_PACKAGE_ROOT'),
      this.configService?.get<string>('PLUGIN_PACKAGE_ROOTS'),
    ]
    ...
    const defaultRoots = [
      resolve(process.cwd(), '.kt-workspace', 'plugin-packages'),
      resolve(process.cwd(), 'src', 'modules', 'plugins'),
    ];
  ...
  private getMaxPackageBytes() {
    const configured = Number(
      this.configService?.get<string>('PLUGIN_PACKAGE_MAX_BYTES'),
    );
  ...
  private normalizeContentHash(contentHash: unknown) {
    if (typeof contentHash !== 'string' || !contentHash) {
      throwVbenError('Bot 插件包缺少 contentHash');
    }
    return contentHash;
  }
}
