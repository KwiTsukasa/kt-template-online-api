...
import { PluginPackageSourceService } from '../package/plugin-package-source.service';
import type {
  PluginPackageDescriptor,
  PluginRuntimeConfigSnapshot,
} from '../package/plugin-package.types';
import { PluginHostBridgeService } from './plugin-host-bridge.service';
import {
  createPluginBullmqWorkerQueueOptions,
  PluginBullmqPluginWorkerRequestQueue,
...
import type { PluginRuntimeFactory } from '@/modules/plugin-platform/application/plugin-platform.service';
import type {
  PluginInstallation,
  PluginVersion,
...
type PluginWorkerThreadDriverOptions = {
...
@Injectable()
export class PluginWorkerRuntimeFactoryService implements PluginRuntimeFactory {
  constructor(
    private readonly configService: ConfigService,
    private readonly packageSource: PluginPackageSourceService,
    private readonly hostBridge: PluginHostBridgeService,
  ) {}

  /**
   * 根据`installation`、`version`构造Bot插件运行态记录。
   * @param installation - 用于Bot插件运行态记录的领域对象，包含 `installedPath`、`id` 字段。
   * @param version - 用于Bot插件运行态记录的领域对象，包含 `manifestJson` 字段。
   * @returns 完成初始化并携带当前边界配置的Bot插件运行态记录。
   */
  create(
    ...
    const driver = new PluginWorkerThreadDriver(this.hostBridge, {
      configSnapshot,
      descriptor,
    ...
    return new PluginWorkerRuntime(
      new PluginBullmqPluginWorkerRequestQueue(
        driver,
        createPluginBullmqWorkerQueueOptions(
          this.configService,
...
export class PluginWorkerThreadDriver implements PluginWorkerDriver {
  private readonly pendingRequests = new Map<string, PendingRequest>();
  private worker?: Worker;

  constructor(
    private readonly hostBridge: PluginHostBridgeService,
    private readonly options: PluginWorkerThreadDriverOptions,
  ) {}

  /**
   * 按`message`投递Bot插件线程驱动记录。
   * @param message - 包含正文、发送目标与账号身份的待处理消息，包含 `correlationId` 字段。
   * @returns 完成初始化并携带当前边界配置的Bot插件线程驱动记录。
   */
  ...
  async dispose(): Promise<void> {
    ...
    this.rejectPending(new Error('Bot 插件 worker 已关闭'));
    if (worker) {
  ...
  private ensureWorker() {
    ...
    worker.on('exit', (code) => {
      this.worker = undefined;
      if (code !== 0) {
        this.rejectPending(new Error(`Bot 插件 worker 异常退出：${code}`));
      }
    });
    this.worker = worker;
    return worker;
  ...
  private settleWorkerResponse(
    ...
    pending.reject(new PluginWorkerResponseError(message.error || {}));
  }
