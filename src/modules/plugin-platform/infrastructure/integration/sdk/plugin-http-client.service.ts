...
export type PluginResolveRedirectRequest = {
  context?: string;
  failureMessage?: (statusCode: number) => string;
  headers?: Record<string, string>;
  maxRedirects?: number;
...
export type PluginRedirectResult = {
  finalUrl: string;
  redirects: string[];
};

@Injectable()
export class PluginHttpClientService {
  /**
  ...
  async resolveRedirect(
    input: PluginResolveRedirectRequest,
  ): Promise<PluginRedirectResult> {
   ...
   */
  async requestJson<T>(input: PluginHttpClientRequest): Promise<T> {
    ...
    return new Promise<Buffer>((resolve, reject) => {
      ...
      const request = client.request(
        url,
        {
          headers: {
            Accept: '*/*',
            'User-Agent': 'kt-template-online-api/plugin',
            ...(input.headers || {}),
          },
          method,
          timeout: timeoutMs,
