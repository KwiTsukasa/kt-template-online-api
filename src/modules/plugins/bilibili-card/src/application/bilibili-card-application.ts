...
export class BilibiliCardApplication {
  ...
  /**
   * 从平台无关事件中解析首个有效 Bilibili 链接，并返回文本回复意图而不直接发送消息。
   * @param message - 只包含 opaque 会话键、正文和链接的插件事件。
   * @returns 是否处理及待由 Bot 适配器发送的回复意图。
   */
  async handleMessage(
    message: BilibiliCardMessage,
  ): Promise<BotPluginEventResult> {
    if (message.isSelf) return emptyEventResult();
    ...
    for (const url of urls) {
      ...
      try {
        ...
        return {
          ...
          replies: [
            {
              content: formatBilibiliVideoReply(video, config),
              kind: 'text',
            },
          ],
        };
 ...
 * @param message - 仅携带 opaque 会话键的跨平台插件消息。
...
/**
 * 构造未处理且无回复的标准插件事件结果，避免各失败分支返回平台相关空值。
 * @returns 固定的空事件结果。
 */
