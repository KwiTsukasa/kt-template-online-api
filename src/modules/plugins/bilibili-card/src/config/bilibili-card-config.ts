...
const CONFIG_RULES = {
  dedupeTtlMs: {
    defaultValue: 600000,
    key: 'PLUGIN_BILIBILI_CARD_DEDUPE_TTL_MS',
    max: 3600000,
    min: 0,
  },
  descMaxLength: {
    defaultValue: 80,
    key: 'PLUGIN_BILIBILI_CARD_DESC_MAX_LENGTH',
    max: 300,
    min: 0,
  },
  httpTimeoutMs: {
    defaultValue: 6000,
    key: 'PLUGIN_BILIBILI_CARD_HTTP_TIMEOUT_MS',
    max: 15000,
    min: 1000,
  },
  maxRedirects: {
    defaultValue: 5,
    key: 'PLUGIN_BILIBILI_CARD_MAX_REDIRECTS',
    max: 10,
    min: 0,
  },
} as const;
