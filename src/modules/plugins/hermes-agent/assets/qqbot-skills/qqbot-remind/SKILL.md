...
提醒始终归属当前发起人，发送到当前聊天；`platformId` 只选择同群提及对象，不改变 Bot 账号、任务所有者或投递群。正文只写普通文本，不拼接 CQ、XML 或 Markdown 提及标签。查询结果会列出已保存的提及对象；给已有提醒补充或变更对象时，先查询原时间和正文、取消原任务，再用确认后的 `platformId` 重建，并核对只有所需任务存续。
