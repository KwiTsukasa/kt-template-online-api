...
import {
  ...
  type QqbotAccountExtension,
} from '@/modules/qqbot/core/application/account/qqbot-account-extension.registry';
...
import type { QqbotMessageDeliveryStatus } from './qqbot-message-subscriber.types';

const SUPERSEDED_STATUSES = ['pending', 'retry'];

@Injectable()
export class QqbotMessageSubscriberAdapter
  implements
    MessageSubscriberAdapter,
    OnModuleDestroy,
    OnModuleInit,
    QqbotAccountExtension
{
  readonly definition = {
    description: '通过 QQBot 账号和 OneBot 目标投递统一消息',
    displayName: 'QQBot',
    subscriberKey: 'qqbot',
    version: 1 as const,
  };
  ...
  /**
   * 将订阅停用或身份变化映射为 QQBot 私有未完成投递的取消状态。
  ...
  async cancelSubscriptionDeliveries(
    ...
    const statuses: QqbotMessageDeliveryStatus[] = ['pending', 'retry'];
   ...
   * @param accountId - 需要清理消息订阅资源的 QQBot 账号标识。
   */
  ...
  /**
   * 接收消息管理已转换的全部模板结果，并按 QQBot 私有目标策略生成投递快照。
  ...
  /**
   * 在通用订阅删除或切换订阅者前检查 QQBot 私有绑定，防止账号配置成为悬空引用。
   * @param manager - 执行引用查询的实体管理器。
   * @param subscriptionId - 需要检查的通用消息订阅标识。
   * @returns 存在至少一个 QQBot 有效配置时返回 true。
   */
  ...
  /**
   * 委托 QQBot 私有执行器在当前批次内发送已冻结投递。
   * @param now - 本轮租约、重试与过期判断的统一时间基准。
   * @returns 本轮 QQBot 实际领取的投递数量。
   */
  ...
  /**
   * 将同一事件与订阅下尚未完成的 QQBot 投递标记为已被取代。
   * @param manager - 执行 QQBot 投递状态更新的实体管理器。
  ...
  /**
   * 将统一协议列出的历史事件对应 QQBot 投递标记为已被当前消息取代。
  ...
  /**
   * 将统一协议明确取消的当前消息对应 QQBot 未完成投递标记为已取消。
   * @param manager - 与统一消息路由共享事务的实体管理器。
  ...
  /**
   * 对每个有效 QQBot 账号目标逐一处理统一消息中的全部已转换模板。
  ...
  private async createDeliveries(
    ...
    const accounts = await input.manager.getRepository(QqbotAccount).find({
  ...
  /**
   * 按事件、QQBot 目标和模板唯一键冻结一条私有投递，并把并发冲突视为幂等成功。
   * @param input - 消息管理已经转换全部模板的统一消息输入。
   * @param binding - QQBot 账号与通用订阅的私有配置。
  ...
  private async createDeliveryIfAbsent(
    ...
    const deliveries = input.manager.getRepository(QqbotMessageDelivery);
    const key = {
