...
import { getBlurredImage } from '@/modules/qqbot/plugins/bangdream/src/theme/canvas-blur-image';
...
}: createBlurredTrianglePatternOptions): Promise<Canvas> {
  // Load image
  ...
  for (let row = 0; row < numRows; row++) {
    ...
    for (let col = 0; col < numCols; col++) {
      ...
      if (isFirstColOffset && col === 0) {
        ...
        const mirroredYEnd = Math.min(
          image.height,
          Math.ceil(mirroredTriangleY + (triangleSize * Math.sqrt(3)) / 2),
        );
        ...
        for (let y = mirroredYStart; y < mirroredYEnd; y++) {
          for (let x = 0; x < triangleSize; x++) {
            ...
            const isInTriangle = isInsideEquilateralTriangle(
              triangleX,
              triangleY,
              pixelX,
              pixelY,
            ...
            imageData.data[idx + 1] = Math.min(
              Math.max(imageData.data[idx + 1] * brightnessFactor, 0),
              255,
            );
            imageData.data[idx + 2] = Math.min(
              Math.max(imageData.data[idx + 2] * brightnessFactor, 0),
              255,
            );
          }
        }
...
function isInsideEquilateralTriangle(
  ...
  if (px < halfSize) {
    return (
      py > (halfSize - px) * Math.sqrt(3) &&
      py < triangleHeight + (halfSize - px) * Math.sqrt(3)
    );
  } else {
    return (
      py > (px - halfSize) * Math.sqrt(3) &&
      py < triangleHeight + (px - halfSize) * Math.sqrt(3)
    );
  }
}
