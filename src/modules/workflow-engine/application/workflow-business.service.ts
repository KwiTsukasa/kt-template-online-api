...
export class WorkflowBusinessService implements WorkflowBusinessPort {
  ...
  private async enter(
    ...
    const subjectValid =
      Boolean(context.subjectId) && context.subjectId.length <= 96;
      ...
      async (manager) => {
        ...
        if (submission.formValues)
          validateDefinitionInput(() =>
            validateDataValues(
              process.launchSchema ?? { fields: [] },
              launchValues,
            ),
          );
        const prepared = await process.prepare({
          ...context,
          values: launchValues,
        });
   ...
   */
  private subjectKey(
    reference: WorkflowProcessReference,
    scopeId: string,
    subjectId: string,
  ): string {
    const process = this.processes.resolve(reference);
    return this.digest([
      process.concurrencyGroup ?? reference.key,
      scopeId,
      subjectId,
    ]);
  }
