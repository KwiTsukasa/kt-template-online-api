...
} from '../domain/workflow-message.policy';
...
export class WorkflowMessageService {
  ...
  async receive(
    runId: string,
    delivery: WorkflowMessageDelivery,
  ): Promise<WorkflowMessageReceipt> {
    const { deliveryId, nodeId, executionId, messageId, senderId, values } =
      delivery;
  ...
  private async accept(
    runId: string,
    incoming: WorkflowMessageDelivery | WorkflowMessageIngress,
  ): Promise<WorkflowMessageReceipt> {
    ...
    const locked = await withWorkflowRunLock(
      ...
      (connection) =>
        connection.transaction(async (manager) => {
          ...
          if (ingress) {
            const matches = (run.bpmnState.activeActivities ?? []).filter(
              (item) => {
                if (declaredBpmnMessage(model, item) !== delivery.messageId)
                  return false;
                try {
                  messageCorrelation(
                    model,
                    run.bpmnState,
                    item,
                    { messageId: delivery.messageId, values },
                    run.inputValues,
                    true,
                  );
                  return true;
          ...
          const waiting = run.bpmnState.activeActivities?.find(
            (item) =>
          ...
          const receipt: WorkflowMessageReceipt = {
          ...
          await manager.update(
            WorkflowRun,
            { id: run.id },
            { bpmnState: run.bpmnState, nextWakeAt: new Date() },
          );
          return receipt;
        }),
    );
