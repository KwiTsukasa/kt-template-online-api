...
export function WorkflowEventSubProcess(definition: any, context: any) {
  if (!definition.behaviour?.triggeredByEvent) return SubProcess(definition, context);
  const activity: any = SubProcess(definition, context, WorkflowEventSubProcessBehaviour as any);
  const run = activity.run.bind(activity);
  activity.run = (content) => {
    // 父作用域的正常令牌耗尽后只等待已有处理实例，不能由处理器自身再启动一轮。
    if (!context.getActivities(activity.parent.id).some((sibling: any) => !sibling.triggeredByEvent && sibling.isRunning)) return;
    if (!activity.isRunning) return run(content);
    if (activity.getStartActivities().some((start: any) => start.behaviour.isInterrupting !== false)) return;
    const source = activity.execution?.source as WorkflowEventSubProcessBehaviour;
...
class WorkflowEventSubProcessBehaviour extends (SubProcessBehaviour as any) {
  ...
  constructor(activity: any, context: any) {
    ...
    this.loopCharacteristics = {
      execute: (message: any) => {
        ...
        else if (this.executions.some((execution: any) => execution.executionId === message.content.executionId)) {
          // 旧快照直接以活动根身份执行子流程，恢复时保留该身份和内部待办。
          this.broker.publish('execution', 'execute.legacy.running', { ...message.content, preventComplete: true });
          super.execute({ ...message, content: { ...message.content, isRootScope: false } });
        }
      },
    };
  }
  ...
  _completeExecution(routingKey: string, content: any): void {
    if (content.executionId === this.rootContent?.executionId && routingKey === 'execute.completed') {
      if (this.executions.some((execution: any) => execution.executionId !== content.executionId && !execution.completed)) {
        super._completeExecution('execute.legacy.completed', { ...content, isRootScope: true, preventComplete: false });
        return;
      }
