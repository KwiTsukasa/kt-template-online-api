...
import { migrateBpmnBoundaryState } from './workflow-bpmn-boundary';
...
export async function advanceWorkflowBpmn(
  ...
  const modelSha256 = createHash('sha256')
    ...
    .digest('hex');
  if (checkpoint && checkpoint.modelSha256 !== modelSha256)
    throw new Error('BPMN 恢复快照与发布版本不一致');
  const selectedStart = checkpoint?.messageStart ?? messageStart;
  const entrySelections = { ...checkpoint?.entrySelections };
  const activityParents = new Map(
    Object.entries(checkpoint?.activityParents ?? {}),
  );
  const interruptedScopes = { ...checkpoint?.interruptedScopes };
  const boundaryOccurrences = structuredClone(
    checkpoint?.boundaryOccurrences ?? {},
  );
  ...
  for (const element of elements) {
    ...
    if (
      ...
      (element.$type === 'bpmn:ReceiveTask' &&
        ...
        !elements.some(
          (flow) =>
            flow.$type === 'bpmn:SequenceFlow' && flow.targetRef === element,
        ))
    ) {
      entryGroups.set(element.id, {
        entryId: element.id,
    ...
    if (element.$type === 'bpmn:EventBasedGateway' && element.instantiate) {
      ...
      for (const flow of elements)
        if (flow.$type === 'bpmn:SequenceFlow' && flow.sourceRef === element)
          entryGroups.set(flow.targetRef.id, group);
    }
  }
  const completed = new Map(
    completions.map((result) => [result.executionId, result]),
  );
  if (completed.size !== completions.length)
    throw new Error('同一 BPMN 活动实例不能提交两份结果');
  ...
  const activityScopes = new Map<string, string[]>(
    Object.entries(checkpoint?.activityScopes ?? {}),
  );
  ...
  const expressionScript = (body: string) => ({
    execute: (
      ...
      try {
        callback(null, expressions.resolveExpression(body, scope));
      } catch (error) {
  ...
  executionRoot.rootElements = (model.root.rootElements ?? [])
    .filter((element) => element.$type !== 'bpmn:CorrelationProperty')
    .map((element) => {
      if (
        selectedStart &&
        element.$type === 'bpmn:Process' &&
        element.id !== selectedStart.processId
      ) {
      ...
      collaboration.messageFlows = (element.messageFlows ?? []).filter(
        (flow) =>
          ![flow.sourceRef, flow.targetRef].some(
            (endpoint) =>
              endpoint?.$type === 'bpmn:Participant' && !endpoint.processRef,
          ),
      );
  ...
  const engine = new Engine({
    ...
    elements: createWorkflowBpmnElements(boundaryOccurrences),
    ...
    extensions: {
      kt: (activity: any) => {
        if (
          ...
          activity.behaviour.Service = HostStep;
        const group = entryGroups.get(activity.id);
        return {
          activate: () => {
            if (
              activity.type === 'bpmn:BoundaryEvent' &&
              activity.behaviour.cancelActivity === false &&
              activity.eventDefinitions?.some((event: any) =>
                [
              ...
              activity.broker.subscribeTmp(
                ...
                (_event, message) => {
                  if (
                    ...
                    activity.broker.publish('execution', 'execute.repeat', {
                      ...message.content,
                      repeat: 1,
                    });
                },
            ...
            activity.broker.subscribeTmp(
              ...
              (_event, message) => {
                ...
                if (activity.type === 'bpmn:UserTask' && output?.value)
                  outputs[activity.id] = output.value;
                if (output?.workflowMessage && output.values)
                  outputs[activity.id] = output.values;
                if (
                  content.isRootScope &&
                  activity.behaviour.loopCharacteristics
                )
                  outputs[activity.id] = {
                    ...outputs[activity.id],
                    items: output,
            ...
            if (
              selectedStart &&
              group?.scopeId === selectedStart.processId &&
              group.entryId !== selectedStart.entryId
            ) {
  ...
  const discardOtherEntries = () => {
    ...
    for (const activity of pendingActivities()) {
      ...
      const scopeExecutionId =
        scope?.executionId ?? activityScopes.get(executionId)?.[0];
  ...
  engine.broker.subscribeTmp(
    ...
    (event, message) => {
      ...
      if (
        event === 'activity.end' &&
        model.elements[content.id]?.$type !== 'bpmn:EventBasedGateway'
      ) {
        ...
        if (
          ...
          if (scope?.executionId)
            interruptedScopes[scope.executionId] = element.$parent.id;
        }
        ...
        if (
          group &&
          scope?.executionId &&
          !entrySelections[scope.executionId]
        ) {
          entrySelections[scope.executionId] = group.entryId;
          discardOtherEntries();
        }
      }
      if (event === 'activity.wait' && entryGroups.has(content.id))
        deliveries.push(discardOtherEntries);
      if (content.type === 'bpmn:UserTask' && event === 'activity.wait') {
        const step = readWorkflowBpmnExtension<WorkflowBpmnStep>(
          model.elements[content.id],
          'kt:Step',
        );
        if (step?.kind !== 'human')
          throw new Error(`人工活动 ${content.id} 未配置办理契约`);
        ...
        if (result) {
          ...
          jobs.set(content.executionId, {
            elementId: content.id,
            name: model.elements[content.id]?.name ?? content.id,
            executionId: content.executionId,
            index: content.index,
            step,
            variables: snapshotWorkflowBpmnVariables(
              engine.environment.variables.input,
              outputs,
            ),
            parentExecutionIds: (
              activityScopes.get(content.executionId) ?? []
            ).filter(
              (id) =>
                !content.ktTaskInstance || id !== content.parent?.executionId,
            ),
          });
      }
      ...
      if (transitions.length > WORKFLOW_BPMN_LIMITS.synchronousTransitions) {
        failure = new Error(
  ...
  try {
    if (checkpoint) {
      engine.recover(
        migrateBpmnBoundaryState(checkpoint.engine, model.elements),
      );
      await engine.resume();
    } else await engine.execute();
    discardOtherEntries();
    for (const deliver of deliveries) {
      if (deliveries.length > WORKFLOW_BPMN_LIMITS.synchronousTransitions)
        throw new Error('BPMN 单次结果交付超过安全上限');
      deliver();
      discardInterruptedScopes();
    }
    for (const signal of signals) {
      const activity = pendingActivities().find(
        (item) =>
          item.id === signal.id &&
          item.content.executionId === signal.executionId,
      );
      if (!activity) {
    ...
    if (completions.length || signals.length) {
      for (const activity of pendingActivities()) {
        ...
        if (
          definitions.some(
            (definition: any) =>
              definition.$type === 'bpmn:ConditionalEventDefinition',
          )
        )
          activity.signal({});
      }
    ...
    discardInterruptedScopes();
    const nextWakeAt = timers.executing.reduce<number | null>(
      (earliest, timer) => {
    ...
    for (const activity of pendingActivities()) {
      ...
      const active: WorkflowBpmnActiveActivity = {
        nodeId: activity.id,
        executionId,
        name: model.elements[activity.id]?.name ?? activity.id,
      ...
      const parents =
        activityParents.get(executionId) ??
        activityParents.get(activity.content.parent?.executionId);
    ...
    return {
      checkpoint: {
        modelSha256,
        engine: state,
        activityScopes: Object.fromEntries(activityScopes),
        outputs,
        entrySelections,
        activityParents: Object.fromEntries(activityParents),
        messageStart: selectedStart,
        interruptedScopes,
        boundaryOccurrences,
      },
      jobs: [...jobs.values()],
