...
import { migrateBpmnBoundaryState, repeatingBpmnEvent, WorkflowConcurrentBoundary } from './workflow-bpmn-boundary';
...
export async function advanceWorkflowBpmn(
  ...
  try {
    if (checkpoint) {
      engine.recover(migrateBpmnBoundaryState(checkpoint.engine, model.elements));
      await engine.resume();
