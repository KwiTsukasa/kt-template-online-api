...
import { assetsRootPath } from '@/qqbot/plugins/bangDream/config/runtime-config';
...
export function drawTextOnCanvas(
  ...
  const numCols = Math.ceil(
    (canvas.width + overflow * 2) / (rotatedWidth + letterSpacing),
  );
  const numRows = Math.ceil(
    (canvas.height + overflow * 2) / (rotatedHeight + lineSpacing),
  );
  ...
  for (let i = 0; i <= numRows; i++) {
    ...
    // 计算每一行的起始位置
    const startY =
      i * (rotatedHeight + lineSpacing) - canvas.height / 2 - offsetY;
    ...
    // 计算每一行文本的起始位置，并添加偏移量
    const startX =
      -(numCols / 2) * (rotatedWidth + letterSpacing) +
      (rotatedWidth + letterSpacing) / 2 +
      overflow +
      (i % 2) * ((rotatedWidth + letterSpacing) / 2);
    // 循环绘制每一列的文本
