...
import { predict } from '@/qqbot/plugins/bangDream/tsugu/data/cutoff-predictor';
...
export class Cutoff {
    ...
    if (!forceReadCache) {
      try {
        ...
        return await callAPIAndCacheResponse(
          `${this.getFinalApiUrl(false)}/api/tracker/data?server=${<number>this.server}&event=${this.eventId}&tier=${this.tier}`,
          0,
          3,
        );
      } catch (e) {
        ...
        try {
          return await callAPIAndCacheResponse(
            `${this.getFinalApiUrl(true)}/api/tracker/data?server=${<number>this.server}&event=${this.eventId}&tier=${this.tier}`,
            0,
            3,
          );
        } catch {
  ...
  async initFull() {
    ...
    //rate
    const rateDataList = mainAPI['rates'] as [
      { server: number; type: string; tier: number; rate: number },
    ];
  ...
  predict(): number {
    ...
    try {
      result = predict(cutoff_ts, start_ts, end_ts, this.rate);
    } catch {
      this.predictEP = 0;
  ...
  getDailyIncrement() {
    ...
    if (score.length == 0) {
      for (
        let i = 0;
        i <= this.getDaysOfEvent(this.cutoffs[this.cutoffs.length - 1].time);
        i++
      ) {
        if (
          this.getDaysOfEvent(this.cutoffs[this.cutoffs.length - 1].time) == 0
        ) {
          scoreFinal.push(this.cutoffs[this.cutoffs.length - 1].ep);
          break;
        }
        const avgIncrementValue = Math.round(
          this.cutoffs[this.cutoffs.length - 1].ep /
            this.getDaysOfEvent(this.cutoffs[this.cutoffs.length - 1].time),
        ); // 计算丢失的天数的平均增量
    ...
    for (let i = 0; i < score.length; i++) {
      ...
      if (this.getDaysOfEvent(time[i]) == j) {
        // 如果当天相对于活动而言天数是i，说明数据完整
        if (this.getDaysOfEvent(time[i]) == 0) {
          // 如果是第0天，且有数据，说明第一天虽然不满24小时，但有数据了，就直接把第一天增量设为当天的ep
          scoreFinal.push(score[i]);
          j++;
        } else {
          // 如果是第i天，且有数据，说明当天数据完整，直接用当天的ep减去前一天的ep就是当天的增量
          scoreFinal.push(score[i]);
          j++;
        }
      } else {
        // i跟相对于getDaysOfEvent的结果不一致，说明当天数据不完整，进行插值计算
        // 当i = 0时，就说明要从0开始而不是score[i-1]开始插值
        if (this.getDaysOfEvent(time[i]) > j) {
          // 如果这个数据是大于标记天数的，则说明需要进行插值计算
          const lostDays = this.getDaysOfEvent(time[i]) - j + 1;
          const avgIncrementValue = Math.round(
            (i == 0 ? score[i] - 0 : score[i] - score[i - 1]) / lostDays,
          ); // 计算丢失的天数的平均增量
          for (let ld = 0; ld < lostDays; ld++) {
            scoreFinal.push(
              Math.round(
                i == 0
                  ? 0 + avgIncrementValue * (ld + 1)
                  : score[i - 1] + avgIncrementValue * (ld + 1),
              ),
            ); // 把丢失的天数的数据补全
            dailyIncrementInvaildDays.push(scoreFinal.length - 1); // 记录增量数据不完整的天数位置
            j++; // 增加一天
    ...
    if (score.length != 0) {
      for (
        let i = 0;
        i < cutoffLastDataDays - this.getDaysOfEvent(time[time.length - 1]);
        i++
      ) {
        // 如果tracker最后一个数据日期跟score最后一个数据的日期有差异，说名是尾巴，要处理
        if (score.length == 0) break;
        const avgIncrementValue = Math.round(
        ...
        if (cutoffLastDataDays - this.getDaysOfEvent(time[time.length - 1]) > 1)
    ...
    for (let i = 0; i < scoreFinal.length; i++) {
      ...
      if (i == 0) {
        dailyIncrement.push(
          `${Math.round(scoreFinal[i] / 10000)}${dailyIncrementInvaildDays.includes(i) ? '!' : ''}`,
        );
      } else {
        dailyIncrement.push(
          `${Math.round((scoreFinal[i] - scoreFinal[i - 1]) / 10000)}${dailyIncrementInvaildDays.includes(i) ? '!' : ''}`,
        );
      }
    }
