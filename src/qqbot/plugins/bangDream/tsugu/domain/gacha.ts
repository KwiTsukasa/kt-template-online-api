...
import { BANGDREAM_GACHA_TYPE_NAME } from '@/qqbot/plugins/bangDream/tsugu/domain/bangdream.enum';
...
export class Gacha {
  ...
  async getData(update: boolean = true) {
    ...
    const gachaData = await callAPIAndCacheResponse(
      `${bestdoriUrl}/api/gacha/${this.gachaId}.json`,
      time,
    );
    return gachaData;
  }
  async getBannerImage(): Promise<Image> {
    try {
      ...
      const BannerImageBuffer = await downloadFileCache(
        `${bestdoriUrl}/assets/jp/homebanner_rip/${this.bannerAssetBundleName}.png`,
        false,
      );
  ...
  async getGachaBGImage(
    ...
    try {
      BGImageBuffer = await downloadFileCache(
        `${bestdoriUrl}/assets/${Server[server]}/gacha/screen/${this.resourceName}_rip/bg.png`,
        false,
      );
    } catch {
      BGImageBuffer = await downloadFileCache(
        `${bestdoriUrl}/assets/${Server[server]}/gacha/screen/${this.resourceName}_rip/bg1.png`,
      );
    }
  ...
  async getGachaLogo(
    ...
    const LogoImageBuffer = await downloadFileCache(
      `${bestdoriUrl}/assets/${Server[server]}/gacha/screen/${this.resourceName}_rip/logo.png`,
    );
...
export async function getPresentGachaList(
  ...
  const gachaListMain = mainAPI['gacha'];
