...
import { AreaItem } from '@/qqbot/plugins/bangDream/tsugu/domain/area-item';
import { Event } from '@/qqbot/plugins/bangDream/tsugu/domain/event';
import { difficultyNameList } from '@/qqbot/plugins/bangDream/tsugu/domain/song';
...
export class Player {
  ...
  async initFull(useCache: boolean = false, mode: 0 | 1 | 2 | 3 = 2) {
    ...
    let playerData;
    try {
      playerData = await callAPIAndCacheResponse(
        `${bestdoriUrl}/api/player/${Server[this.server]}/${this.playerId}?mode=${mode}`,
        cacheTime,
        1,
      );
