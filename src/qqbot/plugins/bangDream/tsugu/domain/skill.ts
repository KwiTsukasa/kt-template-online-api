...
export class Skill {
  ...
  getEffectTypes(): Array<string> {
    ...
    let tempTypeList: Array<string> = [];
    if (this.isExist == false) {
      return ['score'];
    }
    if (this.data['activationEffect'] != undefined) {
      for (const i in this.data['activationEffect']['activateEffectTypes']) {
        if (i == 'score') {
          tempTypeList.push(i);
          if (
            this.data['activationEffect']['activateEffectTypes']['score'][
              'activateCondition'
            ] == 'perfect'
          ) {
  ...
  getSkillDescription(): Array<string> {
    ...
    let tempDescription = this.description;
    if (this.data['onceEffect'] != undefined) {
      //如果包含onceEffect(比如恢复)
      //生成回复数值列表(例如' 3/4/5/6/7 ')
      let onceEffectValueList: string = ' ';
      this.data['onceEffect']['onceEffectValue'].forEach(
  ...
  getScoreUpMaxValue(): number {
    ...
    if (this.data['activationEffect'] != undefined) {
      ...
      if (
        this.data['activationEffect']['unificationActivateEffectValue'] !=
        undefined
      ) {
        numbers.push(
          this.data['activationEffect']['unificationActivateEffectValue'],
      ...
      for (const i in this.data['activationEffect']['activateEffectTypes']) {
        this.data['activationEffect']['activateEffectTypes'][i][
          'activateEffectValue'
        ].forEach((element) => {
          if (element != null) {
