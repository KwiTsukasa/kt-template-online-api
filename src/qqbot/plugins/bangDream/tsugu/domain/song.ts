...
export class Song {
  ...
  async getData() {
    const songData = await callAPIAndCacheResponse(
      `${bestdoriUrl}/api/songs/${this.songId}.json`,
    );
    return songData;
  }
  getSongRip(): number {
  ...
  async getSongJacketImage(
    ...
    let jacketImageBuffer = await downloadFile(jacketImageUrl);
    //下载失败自动尝试切换服务器下载
  ...
  getSongJacketImageURL(displayedServerList?: Server[]): string {
    ...
    const jacketImageUrl = `${bestdoriUrl}/assets/${Server[server]}/musicjacket/musicjacket${songRip}_rip/assets-star-forassetbundle-startapp-musicjacket-musicjacket${songRip}-${jacketImageName.toLowerCase()}-jacket.png`;
  ...
  async getSongChart(difficultyId: number): Promise<object> {
    const songChart = await callAPIAndCacheResponse(
      `${bestdoriUrl}/api/charts/${this.songId}/${difficultyName[difficultyId]}.json`,
    );
    return songChart;
  }
...
export function getPresentSongList(
  ...
  const songListMain = mainAPI['songs'];
...
export function getMetaRanking(
  ...
  const songIdList = Object.keys(mainAPI['meta']);
