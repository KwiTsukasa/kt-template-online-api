...
import { fuzzySearchPath } from '@/qqbot/plugins/bangDream/tsugu/runtime/tsugu-config';
...
export function fuzzySearch(keyword: string): FuzzySearchResult {
  ...
  const keywordList = (
    ...
    (item) => item.replace(/^[\"“”『』「」]|[\"“”『』「」]$/g, ''), // 去掉前后可能的中英文引号
  );
...
function isValidRelationStr(_relationStr: string): boolean {
  ...
  return (
    lessThanPattern.test(_relationStr) ||
        ...
        for (let i = 0; i < target[key].length; i++) {
          ...
          if (
            ...
            matches[key].some(
              (m: any) =>
                typeof m === 'string' &&
                m.toLowerCase() === element.toLowerCase(),
            )
          ) {
            matchArray = true;
            break;
          ...
          if (
            ...
            matches[key].some(
              (m: any) => typeof m === 'number' && m === element,
            )
          ) {
            matchArray = true;
            break;
      ...
      // 处理 Object (string, number) 类型
        ...
        if (
          ...
          matches[key].some(
            (m: any) =>
              typeof m === 'string' &&
              m.toLowerCase() === target[key].toLowerCase(),
          )
        ) {
          match = true;
          continue;
        ...
        if (
          ...
          matches[key].some(
            (m: any) => typeof m === 'number' && m === target[key],
          )
        ) {
          match = true;
          continue;
