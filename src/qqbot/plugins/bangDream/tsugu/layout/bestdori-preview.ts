 ...
 */
export async function drawBestdoriPreview(payload, chart) {
  const { id, diff, level, cover } = payload;
  // 根据BPM和beat计算出真实时间
  ...
  timepoints.forEach((tp, i, a) => {
    if (i === 0) a[i].time = 0;
      ...
      a[i].time =
        a[i - 1].time + (tp.beat - a[i - 1].beat) * (60 / a[i - 1].bpm);
  });
  ...
  const generateSim = function (beat, time, lane) {
    notes.forEach((v) => {
      ...
      if (
        ['Single', 'Flick', 'Skill', 'Long', 'Directional'].includes(v.type) &&
        v.beat === beat
      )
        notes.push({
          type: 'Sim',
          time,
          lane: [v.lane, lane].sort((a, b) => a - b),
        });
    });
  };
  chart.forEach((n) => {
    if (n.type === 'Slide' || n.type === 'Long') {
      ...
      for (let i = 0; i < n.connections.length; i++) {
        ...
        if (i === 0) {
          ...
          notes.push({
            ...tick,
            type: tick.flick ? 'Flick' : tick.skill ? 'Skill' : 'Long',
            time,
          });
          generateSim(tick.beat, time, tick.lane);
        } else {
  ...
  const displayNotes = notes.filter((n) =>
    ['Single', 'SingleOff', 'Skill', 'Flick', 'Directional', 'Long'].includes(
      n.type,
    ),
  );
  const chartLength = Math.ceil(
    displayNotes[displayNotes.length - 1].time + 0.25,
  );
  ...
  // 读取音符图片
  const img_notes = {
    Single: await loadImageFromPath(
      assetsRootPath + '/SongChart/note/Single.png',
    ),
    SingleOff: await loadImageFromPath(
      assetsRootPath + '/SongChart/note/SingleOff.png',
    ),
    Flick: await loadImageFromPath(
      assetsRootPath + '/SongChart/note/Flick.png',
    ),
    FlickTop: await loadImageFromPath(
      assetsRootPath + '/SongChart/note/FlickTop.png',
    ),
    Skill: await loadImageFromPath(
      assetsRootPath + '/SongChart/note/Skill.png',
    ),
    ...
    LeftArrow: await loadImageFromPath(
      assetsRootPath + '/SongChart/note/LeftArrow.png',
    ),
    LeftArrowEnd: await loadImageFromPath(
      assetsRootPath + '/SongChart/note/LeftArrowEnd.png',
    ),
    RightArrow: await loadImageFromPath(
      assetsRootPath + '/SongChart/note/RightArrow.png',
    ),
    RightArrowEnd: await loadImageFromPath(
      assetsRootPath + '/SongChart/note/RightArrowEnd.png',
    ),
  };
  // 读取封面
  const coverImg = await (async () => {
    try {
      if (typeof cover === 'string' || Buffer.isBuffer(cover))
  ...
  bpmList.forEach((BPM, index, array) => {
    ...
    do {
      ...
      BPM.time + beat * (60 / BPM.bpm) < nextTime &&
      BPM.time + beat * (60 / BPM.bpm) >= previousTime
    );
  });
  ctx.restore();
  ...
  notes.forEach((note) => {
    switch (note.type) {
      ...
      case 'Tick':
        ...
        if (note.type === 'Flick') {
          ctx.drawImage(
            ...
            y - h,
            laneWidth * 0.6,
            (laneWidth * 0.6 * img_notes.FlickTop.height) /
              img_notes.FlickTop.width,
          );
        }
        break;
      case 'Directional':
        ...
        const arrowImg =
          img_notes[note.direction === 'Left' ? 'LeftArrow' : 'RightArrow'];
        ...
        for (let i = 0; i < note.width; i++) {
          ...
          const x =
            infoAreaWidth +
            arrowCol * originalWidth +
            blockDistance +
            (note.lane + i * direction) * laneWidth;
          const y = height - ((note.time * heightPerSecond) % height) - h / 2;
          ...
          if (i + 1 === note.width) {
            ...
            const arrowEndX =
              direction === 1 ? x + laneWidth : x - laneWidth * 0.4;
            ctx.drawImage(
              endImg,
              arrowEndX,
              y,
      ...
      case 'Sim':
        ...
        const simStartX =
          infoAreaWidth +
          simCol * originalWidth +
          blockDistance +
          (note.lane[0] + 1) * laneWidth;
        const simY =
          height - ((note.time * heightPerSecond) % height) - simH / 2;
      ...
      case 'Bar':
        ...
        for (let i = startCol; i <= endCol; i++) {
          const x1 =
            ...
            i * originalWidth +
            blockDistance +
            note.lane[0] * laneWidth;
          const x2 =
            infoAreaWidth +
            i * originalWidth +
            blockDistance +
            note.lane[1] * laneWidth;
          const y1 = height * (i + 1) - note.time[0] * heightPerSecond;
          const y2 = height * (i + 1) - note.time[1] * heightPerSecond;
