...
describe('NapcatContainerService', () => {
  it('serializes every remote Docker mutation with the runtime migration lock', async () => {
    ...
    const runtime = {
      dataDir:
        '/vol1/docker/kt-bot-adapter/napcat-instances/kt-napcat-container-1',
      id: 'container-1',
      name: 'kt-napcat-container-1',
    };
  ...
  it('removes an unbound create-login container even after the row was marked deleted', async () => {
    const container = {
      accountId: null,
      dataDir: '/vol1/docker/kt-bot-adapter/napcat-instances/kt-napcat-create',
      id: 'container-create',
      isDeleted: true,
      name: 'kt-napcat-create',
    };
    const bindingRepository = {
  ...
  it('does not remove a container that already has an account owner during binding', async () => {
    ...
    const containerRepository = {
      findOne: jest.fn().mockResolvedValue({
        accountId: 'account-1',
        dataDir: '/vol1/docker/kt-bot-adapter/napcat-instances/kt-napcat-bound',
        id: 'container-binding',
        isDeleted: false,
        name: 'kt-napcat-bound',
      }),
      update: jest.fn(),
    };
    const service = new NapcatContainerService(
...
`).offlineReason,
    ...
    const service = new NapcatContainerService(
      {
        get: jest.fn((key: string) => {
          const values: Record<string, string> = {
            NAPCAT_CONTAINER_MODE: 'ssh',
            NAPCAT_RUNTIME_CHECK_TIMEOUT_MS: '5000',
            NAPCAT_SSH_TARGET: 'nas',
          };
          return values[key] || '';
        }),
  ...
  it('injects ACCOUNT and NAPCAT_QUICK_PASSWORD env only when provided', () => {
    ...
    const baseInput = {
      dataDir: '/vol1/docker/kt-bot-adapter/napcat-instances/kt-napcat-test',
  ...
  it('skips quick-login recreate when the container already carries ACCOUNT', async () => {
    const containerRepository = {
      createQueryBuilder: jest.fn(() => ({
        ...
        getOne: jest.fn().mockResolvedValue({
          dataDir: '/vol1/docker/kt-bot-adapter/napcat-instances/kt-napcat-x',
          id: 'container-1',
          image: 'mlikiowa/napcat-docker:latest',
          name: 'kt-napcat-x',
          reverseWsUrl: 'ws://127.0.0.1:48085/bot-adapter/napcat/onebot/reverse',
    ...
    const recreated = await service.ensureRuntimeQuickLogin(
      { id: 'container-1', name: 'kt-napcat-x' },
      '2354598417',
    );
  ...
  it('starts a stopped primary container when login env already matches', async () => {
    ...
    service.runProcess = jest
      ...
      .mockResolvedValueOnce({ stderr: '', stdout: 'kt-napcat-x\n' });
  ...
  it('requires an explicit NapCat image when creating a managed container', async () => {
    ...
    await expect(service.createManagedContainer('1914728559')).rejects.toThrow(
      'NapCat 镜像未配置，请先设置 NAPCAT_IMAGE',
    );
