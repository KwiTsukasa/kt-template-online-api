...
import { NapcatLoginSession } from '@/modules/bot-adapter/napcat/infrastructure/persistence/napcat-login-session.entity';
import { NapcatLoginStateStoreService } from '@/modules/bot-adapter/napcat/infrastructure/persistence/napcat-login-state-store.service';

describe('NapcatLoginService', () => {
  const mockWebuiLoginPost = (
    target: NapcatLoginService,
    options: {
  ...
  const service = new NapcatLoginService(
    { get: jest.fn() } as unknown as ConfigService,
    {} as BotAccountService,
    {} as NapcatContainerService,
    toolsService,
  );
  ...
  it('reuses the existing account container when refreshing login', async () => {
    ...
    const refreshService = new NapcatLoginService(
      { get: jest.fn() } as unknown as ConfigService,
      accountService as unknown as BotAccountService,
      containerService as unknown as NapcatContainerService,
      new ToolsService(),
    );
    ...
    const refreshService = new NapcatLoginService(
      {
        get: jest.fn((key: string) => {
          if (key === 'NAPCAT_LOGIN_QR_EXPIRE_MS') return '120000';
          if (key === 'NAPCAT_PASSWORD_LOGIN_WAIT_MS') return '1000';
          if (key === 'NAPCAT_LOGIN_POLL_INTERVAL_MS') return '1000';
  ...
  it('returns create login scan before remote container startup finishes', async () => {
    ...
    const createService = new NapcatLoginService(
      { get: jest.fn() } as unknown as ConfigService,
      {} as BotAccountService,
      containerService as unknown as NapcatContainerService,
      new ToolsService(),
  ...
  it('recovers stale create login preparation after the background task was lost', async () => {
    ...
    const createService = new NapcatLoginService(
      {
        get: jest.fn((key: string) => {
          if (key === 'NAPCAT_CREATE_PREPARING_STALE_MS') return '1';
          return undefined;
        }),
      } as unknown as ConfigService,
      {} as BotAccountService,
  ...
  it('monitors a ready qrcode session and publishes login success without frontend polling', async () => {
    ...
    const monitorService = new NapcatLoginService(
      {
        get: jest.fn((key: string) =>
          key === 'NAPCAT_LOGIN_POLL_INTERVAL_MS' ? 10 : undefined,
        ),
      } as unknown as ConfigService,
      accountService as unknown as BotAccountService,
      containerService as unknown as NapcatContainerService,
  ...
  it('keeps scan pending when NapCat is login-positive before QQ number is readable', async () => {
    ...
    const loginService = new NapcatLoginService(
      { get: jest.fn() } as unknown as ConfigService,
      accountService as unknown as BotAccountService,
      containerService as unknown as NapcatContainerService,
      new ToolsService(),
  ...
  it('does not let the server-side qrcode monitor extend the original qrcode deadline on temporary errors', async () => {
    ...
    const monitorService = new NapcatLoginService(
      {
        get: jest.fn((key: string) => {
          if (key === 'NAPCAT_LOGIN_POLL_INTERVAL_MS') return 10;
      ...
      {
        ensureScannedAccount: jest.fn().mockResolvedValue('account-1'),
      } as unknown as BotAccountService,
      {
        bindAccount: jest.fn().mockResolvedValue(undefined),
      } as unknown as NapcatContainerService,
      new ToolsService(),
    );
    const session = (monitorService as any).createSession({
      accountId: 'account-1',
  ...
  it('refreshes the monitor deadline when a new qrcode is published while a timer is already active', async () => {
    ...
    const monitorService = new NapcatLoginService(
      {
        get: jest.fn((key: string) => {
          if (key === 'NAPCAT_LOGIN_POLL_INTERVAL_MS') return 60;
    ...
    const refreshService = new NapcatLoginService(
      {
        get: jest.fn((key: string) => {
          const values: Record<string, string> = {
            NAPCAT_LOGIN_POLL_INTERVAL_MS: '1',
            NAPCAT_PASSWORD_LOGIN_WAIT_MS: '3',
            NAPCAT_QUICK_LOGIN_WAIT_MS: '1',
          };
          return values[key] || '';
        }),
      } as unknown as ConfigService,
    ...
    const refreshService = new NapcatLoginService(
      {
        get: jest.fn((key: string) => {
          const values: Record<string, string> = {
            NAPCAT_LOGIN_POLL_INTERVAL_MS: '1',
            NAPCAT_PASSWORD_LOGIN_WAIT_MS: '1',
            NAPCAT_QUICK_LOGIN_WAIT_MS: '1',
          };
          return values[key] || '';
        }),
    ...
    const refreshService = new NapcatLoginService(
      {
        get: jest.fn((key: string) => {
          const values: Record<string, string> = {
            NAPCAT_LOGIN_POLL_INTERVAL_MS: '1',
            NAPCAT_PASSWORD_LOGIN_WAIT_MS: '10',
            NAPCAT_QUICK_LOGIN_WAIT_MS: '1',
          };
          return values[key] || '';
        }),
    ...
    const refreshService = new NapcatLoginService(
      {
        get: jest.fn((key: string) => {
          const values: Record<string, string> = {
            NAPCAT_LOGIN_POLL_INTERVAL_MS: '1000',
            NAPCAT_PASSWORD_LOGIN_WAIT_MS: '3000',
          };
          return values[key] || '';
        }),
      } as unknown as ConfigService,
  ...
  it('persists expired scan sessions before removing them from the runtime cache', async () => {
    ...
    const expireService = new NapcatLoginService(
      { get: jest.fn() } as unknown as ConfigService,
      {} as BotAccountService,
      {
  ...
  it('persists failed scan sessions before cleaning runtime state', async () => {
    ...
    const failService = new NapcatLoginService(
      { get: jest.fn() } as unknown as ConfigService,
      {
  ...
  it('refreshes a qrcode that is too close to the native QQ expiry window', async () => {
    const expiringService = new NapcatLoginService(
      {
        get: jest.fn((key: string) => {
          if (key === 'NAPCAT_LOGIN_NATIVE_QR_EXPIRE_MS') return '120000';
          if (key === 'NAPCAT_LOGIN_QR_SAFE_SCAN_MS') return '45000';
  ...
  it('syncs account qrcode status when scan status reads a NapCat qrcode', async () => {
    ...
    const syncService = new NapcatLoginService(
      { get: jest.fn() } as unknown as ConfigService,
      accountService as unknown as BotAccountService,
      {} as NapcatContainerService,
      new ToolsService(),
