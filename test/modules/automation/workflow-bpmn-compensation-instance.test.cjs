...
const step = (id, human = false, extra = {}) => ({
...
const makeModel = async (human = false) => {
  const transaction = {
    ...
    id: 'Tx',
    flowElements: [
      { $type: 'bpmn:StartEvent', id: 'TxStart' },
  ...
  const model = await parseWorkflowBpmn({
    ...
    model: {
      ...
      id: 'Definitions',
      targetNamespace: 'urn:kt:compensation-instance',
      rootElements: [
        {
          $type: 'bpmn:Process',
          id: 'Process',
...
const cancelAndCompensate = async (model, result, expected) => {
  ...
  const compensations = result.jobs
    .filter((job) => job.elementId === 'Undo')
  ...
  for (const executionId of compensations)
    result = await finish(
      model,
      result,
      result.jobs.filter((job) => job.executionId === executionId),
    );
  assert.deepEqual(
...
for (const human of [false, true])
  for (const completed of [0, 1, 2]) {
    test(`${human ? '人工' : '服务'}任务两个并发实例中${completed}个完成：仅补偿成功实例，恢复不重复执行`, async () => {
      ...
      const hostIds = result.jobs
        .filter((job) => job.elementId === 'Host')
        .slice(0, completed)
        .map((job) => job.executionId);
      for (const executionId of hostIds)
        result = await finish(
          model,
          result,
          result.jobs.filter((job) => job.executionId === executionId),
...
test('旧版已保存的补偿队列过滤容器完成，保留两个成功实例及其原生回执', async () => {
  ...
  let result;
  try {
    adapter.WorkflowCompensateEventDefinition =
      require('bpmn-elements').CompensateEventDefinition;
    ...
    adapter.WorkflowCompensateEventDefinition = current;
  }
...
const makeLoopModel = async (mode, cardinality = 3) => {
  ...
  const transaction = document.model.rootElements[0].flowElements.find(
    (item) => item.id === 'Tx',
  );
  transaction.flowElements = transaction.flowElements.filter(
    (item) => !['A', 'B', 'Fork_B', 'B_Host', 'A_Host'].includes(item.id),
  );
  const entrance = transaction.flowElements.find(
    (item) => item.id === 'Fork_A',
  );
  ...
  if (mode === 'standard')
    loop = {
      ...
      testBefore: false,
      loopMaximum: cardinality,
    };
  transaction.flowElements.find(
    (item) => item.id === 'Host',
...
for (const mode of ['parallel', 'sequential', 'standard'])
  for (const outcome of ['completed', 'cancelled', 'error', 'legacy']) {
    test(`${mode} 循环 ${outcome} 时按外层活动成功状态补偿，不能把每次迭代额外派发为补偿`, async () => {
      ...
      try {
        ...
        if (outcome === 'completed' || outcome === 'legacy') {
          for (
            let count = 0;
            count < 3 && result.jobs.some((job) => job.elementId === 'Host');
            count += 1
          )
            result = await finish(
              model,
        ...
        if (outcome === 'error') {
          ...
          result = await advanceWorkflowBpmn(model, restore(result), {}, [
            {
              executionId: host.executionId,
              error: { code: 'BAD', message: 'loop rejected' },
            },
          ]);
        }
      } finally {
...
for (const mode of ['parallel', 'sequential'])
  test(`零次${mode}多实例活动成功完成，关联补偿只执行一次`, async () => {
