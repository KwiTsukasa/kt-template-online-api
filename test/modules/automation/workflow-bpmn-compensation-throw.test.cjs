...
const makeModel = async (target = 'A', wait = true) => {
  const event = { $type: 'bpmn:CompensateEventDefinition', waitForCompletion: wait };
  if (target) event.activityRef = ref(target);
  const process = { $type: 'bpmn:Process', id: 'Process', isExecutable: true, flowElements: [
    { $type: 'bpmn:StartEvent', id: 'Start' }, step('A'), step('B'),
    ...
    { $type: 'bpmn:IntermediateThrowEvent', id: 'Throw', eventDefinitions: [event] }, step('After'),
    { $type: 'bpmn:EndEvent', id: 'End' }, flow('Start', 'A'), flow('A', 'B'), flow('B', 'Throw'), flow('Throw', 'After'), flow('After', 'End'),
  ...
  const model = await parseWorkflowBpmn({ format: 'bpmn20', model: { $type: 'bpmn:Definitions', id: 'Definitions', targetNamespace: 'urn:kt:compensation-throw', rootElements: [process] } });
...
for (const rounds of [1, 3]) test(`回环图执行 ${rounds} 轮后逐实例逆序补偿，恢复不重新派发已完成的处理器`, async () => {
  const document = (await makeModel(null)).definition;
  const process = document.model.rootElements[0];
  process.flowElements = process.flowElements.filter(item => item.id !== 'B_Throw');
  process.flowElements.push({ $type: 'bpmn:ExclusiveGateway', id: 'Repeat', default: ref('Repeat_Throw') }, flow('B', 'Repeat'), flow('Repeat', 'Throw'), {
  ...
  for (let index = 0; index < rounds; index++) {
    ...
    result = await advanceWorkflowBpmn(model, restore(result), {}, [{ executionId: result.jobs[0].executionId, output: { again: index + 1 < rounds } }]);
  }
  for (let index = 0; index < rounds; index++) {
...
test('补偿失败时不提前派发后续步骤，保持未捕获错误的失败结果', async () => {
  ...
  assert.ok(!result.transitions.some(event => event.elementId === 'After' && event.event === 'activity.enter'));
});
...
test('无依赖的并行活动可以并行补偿，全部结束才继续', async () => {
  const document = (await makeModel(null)).definition;
  const process = document.model.rootElements[0];
  process.flowElements = process.flowElements.filter(item => !['Start_A', 'A_B', 'B_Throw'].includes(item.id));
  process.flowElements.push({ $type: 'bpmn:ParallelGateway', id: 'Fork' }, { $type: 'bpmn:ParallelGateway', id: 'Join' }, flow('Start', 'Fork'), flow('Fork', 'A'), flow('Fork', 'B'), flow('A', 'Join'), flow('B', 'Join'), flow('Join', 'Throw'));
  const model = await parseWorkflowBpmn(document);
  assert.deepEqual(validateWorkflowBpmn(model), []);
  let result = await advanceWorkflowBpmn(model, null, {});
...
for (const wait of [true, false]) test(`补偿结束事件 waitForCompletion=${wait} 保留处理器身份并完成流程`, async () => {
  const document = (await makeModel('A', wait)).definition;
  const process = document.model.rootElements[0];
  process.flowElements = process.flowElements.filter(item => !['After', 'End', 'Throw_After', 'After_End'].includes(item.id));
  process.flowElements.find(item => item.id === 'Throw').$type = 'bpmn:EndEvent';
  const model = await parseWorkflowBpmn(document);
  assert.deepEqual(validateWorkflowBpmn(model), []);
  let result = await start(model);
  assert.deepEqual(ids(result), ['UndoA']);
...
test('子流程抛出的无目标补偿只处理子流程实例，不能补偿父作用域的活动', async () => {
  ...
  const inner = { $type: 'bpmn:SubProcess', id: 'Inner', flowElements: [
    { $type: 'bpmn:StartEvent', id: 'InnerStart' }, ...process.flowElements.filter(item => innerIds.includes(item.id)),
    { $type: 'bpmn:EndEvent', id: 'InnerEnd' }, flow('InnerStart', 'B'), flow('Throw', 'InnerEnd'),
  ], artifacts: process.artifacts.filter(item => item.id === 'LinkB') };
  process.flowElements = process.flowElements.filter(item => !innerIds.includes(item.id) && !['A_B', 'Throw_After'].includes(item.id));
