...
const event = (id, type, definition, attributes = {}) => ({
  $type: `bpmn:${type}`,
  id,
  ...attributes,
  ...(definition && { eventDefinitions: [definition] }),
});
const signal = () => ({
  $type: 'bpmn:SignalEventDefinition',
  signalRef: { $ref: 'Signal' },
});
const escalation = () => ({
  $type: 'bpmn:EscalationEventDefinition',
  escalationRef: { $ref: 'Escalation' },
});
const restore = (result) => JSON.parse(JSON.stringify(result.checkpoint));
const ids = (result) => result.jobs.map((job) => job.elementId).sort();
const complete = (model, result, id) =>
  advanceWorkflowBpmn(model, restore(result), {}, [
    {
      executionId: result.jobs.find((job) => job.elementId === id).executionId,
      output: {},
    },
  ]);
const definition = (body) => ({
  ...
  model: {
    ...
    id: 'Definitions',
    targetNamespace: 'urn:kt:event-scope',
    rootElements: [
      { $type: 'bpmn:Signal', id: 'Signal' },
      { $type: 'bpmn:Escalation', id: 'Escalation', escalationCode: 'REVIEW' },
      {
        $type: 'bpmn:Process',
        id: 'Process',
        isExecutable: true,
...
const handler = (interrupting) => ({
  $type: 'bpmn:SubProcess',
  id: 'Handler',
  triggeredByEvent: true,
  flowElements: [
    event('HandleStart', 'StartEvent', signal(), {
      isInterrupting: interrupting,
    }),
    human('Review'),
    event('HandleEnd', 'EndEvent'),
    flow('HandleStart', 'Review'),
    flow('Review', 'HandleEnd'),
  ],
});
const broadcastBody = () => [
  ...
  human('Trigger1'),
  human('Trigger2'),
  event('Broadcast1', 'IntermediateThrowEvent', signal()),
  event('Broadcast2', 'IntermediateThrowEvent', signal()),
  event('End', 'EndEvent'),
  flow('Start', 'Fork'),
  flow('Fork', 'Main'),
  flow('Fork', 'Trigger1'),
  flow('Main', 'End'),
  flow('Trigger1', 'Broadcast1'),
  flow('Broadcast1', 'Trigger2'),
  flow('Trigger2', 'Broadcast2'),
  flow('Broadcast2', 'End'),
];
...
test('事件子流程拒绝外部顺序流、附着边界、缺失或重复开始事件以及非中断错误捕获', async () => {
  const invalid = [
    ...
    [
      ...
      (body) =>
        body.push(
          event('InvalidBoundary', 'BoundaryEvent', signal(), {
            attachedToRef: { $ref: 'Handler' },
          }),
        ),
    ],
    [
      ...
      (body) =>
        body
          .at(-1)
          .flowElements.push(event('AnotherStart', 'StartEvent', signal())),
    ],
    [
      'event-subprocess-trigger',
      (body) => delete body.at(-1).flowElements[0].eventDefinitions,
    ],
    [
      'event-subprocess-error',
      (body) =>
        (body.at(-1).flowElements[0].eventDefinitions = [
...
test('非中断事件子流程允许重叠处理，JSON 恢复保留所有实例且父流程等待处理完毕', async () => {
  ...
  const first = result.jobs.find(
    (job) => job.elementId === 'Review',
  ).executionId;
  result = await complete(model, result, 'Trigger2');
  assert.deepEqual(ids(result), ['Main', 'Review', 'Review']);
  const reviews = result.jobs
    .filter((job) => job.elementId === 'Review')
  ...
  result = await advanceWorkflowBpmn(
    ...
    restore(result),
    {},
    reviews.map((executionId) => ({ executionId, output: {} })),
  );
...
test('非中断信号边界重复触发但不撤销宿主，宿主结束后仍等待已生成的分支', async () => {
  const model = await modelFor([
    ...broadcastBody(),
    event('Boundary', 'BoundaryEvent', signal(), {
      attachedToRef: { $ref: 'Main' },
...
for (const interrupting of [true, false])
  test(`升级边界捕获${interrupting ? '中断' : '保留'}子流程，恢复不重新抛出升级`, async () => {
    const sub = {
      $type: 'bpmn:SubProcess',
      id: 'Sub',
      flowElements: [
        event('SubStart', 'StartEvent'),
        human('Trigger'),
        event('Raise', 'IntermediateThrowEvent', escalation()),
        human('Inner'),
        event('SubEnd', 'EndEvent'),
        flow('SubStart', 'Trigger'),
        flow('Trigger', 'Raise'),
        flow('Raise', 'Inner'),
        flow('Inner', 'SubEnd'),
      ],
    ...
    const model = await modelFor([
      event('Start', 'StartEvent'),
      sub,
      event('Boundary', 'BoundaryEvent', escalation(), {
    ...
    assert.deepEqual(
      ids(result),
      interrupting ? ['Review'] : ['Inner', 'Review'],
    );
...
test('同一同步链连续抛出两次信号时，非中断边界不能漏掉后一次', async () => {
  const body = broadcastBody().filter(
    (item) =>
      item.id !== 'Trigger2' &&
      item.id !== 'Broadcast1_Trigger2' &&
      item.id !== 'Trigger2_Broadcast2',
  );
  const model = await modelFor([
    ...body,
    flow('Broadcast1', 'Broadcast2'),
    event('Boundary', 'BoundaryEvent', signal(), {
      attachedToRef: { $ref: 'Main' },
      cancelActivity: false,
    }),
...
test('人工事件处理实例恢复后仍能读取原业务输入', async () => {
  const model = await modelFor([...broadcastBody(), handler(false)]);
  let result = await advanceWorkflowBpmn(model, null, {
    input: { task: 'fixed-business' },
  });
  result = await complete(model, result, 'Trigger1');
  assert.deepEqual(
    result.jobs.find((job) => job.elementId === 'Review').variables.input,
    { task: 'fixed-business' },
  );
  result = await advanceWorkflowBpmn(model, restore(result), {});
  for (const job of result.jobs)
    assert.deepEqual(job.variables.input, { task: 'fixed-business' });
});

test('父作用域正常令牌全部完成后，仅等待已有事件处理，不再创建新的处理实例', async () => {
  const sub = handler(false);
  sub.flowElements = sub.flowElements.filter(
    (item) => item.id !== 'Review_HandleEnd',
  );
  sub.flowElements.push(
    event('Again', 'IntermediateThrowEvent', signal()),
    flow('Review', 'Again'),
    flow('Again', 'HandleEnd'),
  );
  ...
  result = await advanceWorkflowBpmn(
    ...
    restore(result),
    {},
    original.map((executionId) => ({ executionId, output: {} })),
  );
  assert.equal(result.status, 'succeeded');
...
test('升级前的普通人工任务 JSON 快照保留原执行身份且只推进一次', async () => {
  ...
  const engine = new Engine({
    moddleContext: {
      rootElement: model.root,
      elementsById: model.elements,
      references: model.references,
      warnings: [],
    },
    variables: { input: { business: 'before-upgrade' } },
  });
  await engine.execute();
  const original = engine.execution.getPostponed()[0].content.executionId;
  const checkpoint = {
    modelSha256: createHash('sha256')
      .update(JSON.stringify(model.definition.model))
      .digest('hex'),
    engine: await engine.getState(),
    activityScopes: {},
    outputs: {},
  };
  await engine.stop();
  let result = await advanceWorkflowBpmn(
    model,
  ...
  result = await advanceWorkflowBpmn(model, restore(result), {}, [
    { executionId: original, output: { confirmed: true } },
  ]);
  assert.equal(result.status, 'succeeded');
  assert.equal(
    result.transitions.filter(
      (item) => item.event === 'flow.take' && item.elementId === 'Legacy_End',
    ).length,
    1,
  );
});
...
for (const legacyFirst of [true, false])
  test(`升级前的事件子流程保留原待办，新旧处理先后完成顺序 ${legacyFirst}`, async () => {
    const { Engine } = require('bpmn-engine');
    const { createHash } = require('node:crypto');
    const model = await modelFor([...broadcastBody(), handler(false)]);
    const engine = new Engine({
    ...
    const original = engine.execution
      .getPostponed()
      .find((item) => item.id === 'Handler')
      .getPostponed()
      .find((item) => item.id === 'Review').content.executionId;
    const checkpoint = {
      modelSha256: createHash('sha256')
        .update(JSON.stringify(model.definition.model))
        .digest('hex'),
    ...
    assert.deepEqual(ids(result), ['Main', 'Review', 'Trigger2']);
    assert.equal(
      result.jobs.find((job) => job.elementId === 'Review').executionId,
      original,
    );
    result = await complete(model, result, 'Trigger2');
    assert.deepEqual(ids(result), ['Main', 'Review', 'Review']);
    const first = legacyFirst
      ? original
      : result.jobs.find(
          (job) => job.elementId === 'Review' && job.executionId !== original,
        ).executionId;
    result = await advanceWorkflowBpmn(model, restore(result), {}, [
      { executionId: first, output: {} },
    ]);
...
test('同一服务节点的重复边界令牌各有执行身份，结果恢复和出口均只消费一次', async () => {
  ...
  result = await complete(model, result, 'Trigger1');
  result = await complete(model, result, 'Trigger2');
  const workIds = result.jobs
    .filter((job) => job.elementId === 'Work')
    .map((job) => job.executionId);
  assert.equal(new Set(workIds).size, 2);
  result = await advanceWorkflowBpmn(model, restore(result), {}, [
    { executionId: workIds[0], output: { receipt: 'one' } },
  ]);
  assert.deepEqual(
    result.jobs
      .filter((job) => job.elementId === 'Work')
  ...
  assert.equal(
    result.transitions.filter(
      (item) => item.event === 'flow.take' && item.elementId === 'Work_End',
    ).length,
    1,
  );
  result = await advanceWorkflowBpmn(model, restore(result), {}, [
    { executionId: workIds[0], output: { receipt: 'duplicate' } },
  ]);
...
test('多实例子流程中的中断事件仅撤销收到该事件的实例，其他实例仍可继续', async () => {
  const sub = {
    ...
    flowElements: [
      ...broadcastBody().map((item) =>
        item.id === 'Broadcast1' || item.id === 'Broadcast2'
          ? { ...item, eventDefinitions: [escalation()] }
          : item,
      ),
      {
        ...handler(true),
        flowElements: handler(true).flowElements.map((item) =>
          item.id === 'HandleStart'
            ? { ...item, eventDefinitions: [escalation()] }
            : item,
        ),
  ...
  const model = await modelFor([
    event('OuterStart', 'StartEvent'),
    sub,
    event('OuterEnd', 'EndEvent'),
    flow('OuterStart', 'Outer'),
    flow('Outer', 'OuterEnd'),
  ]);
  let result = await advanceWorkflowBpmn(model, null, {});
  ...
  assert.equal(
    originalMains.filter((job) =>
      result.cancelledExecutionIds.includes(job.executionId),
    ).length,
    1,
  );
  result = await complete(model, result, 'Review');
...
test('升级前的服务任务 JSON 快照保持原脚本身份，结果仅交付一次', async () => {
    ...
    extensions: {
      legacy: (activity) => {
        if (activity.type === 'bpmn:ServiceTask')
          activity.behaviour.Service = class {
            execute() {}
          };
      },
    },
  ...
  assert.equal(result.jobs[0].executionId, original);
  result = await advanceWorkflowBpmn(model, restore(result), {}, [
    { executionId: original, output: { done: true } },
  ]);
  assert.equal(result.status, 'succeeded');
  assert.equal(
    result.transitions.filter(
