...
const makeDocument = () => ({
  ...
  model: {
    ...
    id: 'Definitions',
    targetNamespace: 'urn:kt:transaction-compensation',
    rootElements: [
      {
        $type: 'bpmn:Process',
        id: 'Process',
        ...
        flowElements: [
          ...
          },
          {
            $type: 'bpmn:BoundaryEvent',
            id: 'Cancelled',
            attachedToRef: ref('Transaction'),
            eventDefinitions: [{ $type: 'bpmn:CancelEventDefinition' }],
          },
          step('Recovered'),
          { $type: 'bpmn:EndEvent', id: 'End' },
          flow('Start', 'Transaction'),
...
const start = async (model) => {
  ...
  for (const name of ['A', 'B', 'Trigger'])
    result = await finish(model, result, [name]);
  return result;
};
...
test('事务取消立即撤销还在执行的同级活动，补偿等待不允许该活动继续', async () => {
  ...
  assert.deepEqual(ids(result), ['UndoB']);
  result = await advanceWorkflowBpmn(model, restore(result), {}, [
    { executionId: running.executionId, output: { late: true } },
  ]);
...
test('事务补偿失败保持失败结果，不能进入取消恢复路径或派发前序补偿', async () => {
  ...
  assert.ok(
    !result.transitions.some(
      (item) =>
        ['UndoA', 'Recovered'].includes(item.elementId) &&
        item.event === 'activity.enter',
    ),
  );
});
...
test('事务补偿错误可由事务外的错误边界捕获，不误走取消出口', async () => {
  ...
  document.model.rootElements[0].flowElements.push(
    ...
    step('HandleFault'),
    flow('Fault', 'HandleFault'),
    flow('HandleFault', 'End'),
  );
  const model = await makeModel(document);
  let result = await start(model);
  result = await advanceWorkflowBpmn(model, restore(result), {}, [
...
test('正常完成的事务不启动补偿也不走取消出口', async () => {
  ...
  delete transaction.flowElements.find((item) => item.id === 'Cancel')
    .eventDefinitions;
  const model = await makeModel(document);
  ...
  assert.ok(
    !result.transitions.some(
      (item) =>
        ['UndoA', 'UndoB', 'Recovered'].includes(item.elementId) &&
        item.event === 'activity.enter',
    ),
  );
});
...
test('回环事务按实际完成顺序逐实例逆序补偿，阶段切换不丢失剩余边界', async () => {
  ...
  for (let index = 0; index < 3; index++) {
    result = await finish(model, result, ['A']);
    result = await advanceWorkflowBpmn(model, restore(result), {}, [
      { executionId: result.jobs[0].executionId, output: { again: index < 2 } },
    ]);
  }
  result = await finish(model, result, ['Trigger']);
  for (let index = 0; index < 3; index++) {
...
for (const cancelled of [false, true])
  test(`旧原生事务检查点取消状态=${cancelled} 恢复已有工作身份并完成`, async () => {
    ...
    try {
      ...
      adapter.WorkflowTransaction = factory;
    }
