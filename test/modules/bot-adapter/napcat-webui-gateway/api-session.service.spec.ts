...
} from '../../../../src/modules/bot-adapter/napcat/webui-gateway/application/napcat-webui-gateway.service';
import { NapcatWebuiGatewayController } from '../../../../src/modules/bot-adapter/napcat/webui-gateway/contract/napcat-webui-gateway.controller';
import { NapcatWebuiGatewayClient } from '../../../../src/modules/bot-adapter/napcat/webui-gateway/infrastructure/napcat-webui-gateway.client';
import { NapcatWebuiGatewayAudit } from '../../../../src/modules/bot-adapter/napcat/webui-gateway/infrastructure/persistence/napcat-webui-gateway-audit.entity';
...
} from '../../../../src/modules/bot-adapter/napcat/infrastructure/persistence';
...
const WEBUI_PERMISSION = 'Bot:Account:WebUI';
...
describe('NapcatWebuiGatewayController', () => {
  ...
  beforeAll(async () => {
    const moduleRef = await Test.createTestingModule({
      controllers: [NapcatWebuiGatewayController],
      providers: [
        {
          provide: NapcatWebuiGatewayService,
  ...
  it('denies non-super Admin users without WebUI permission', async () => {
    currentAdminUser = createAdminUser([
      {
        menus: [{ authCode: 'Bot:Account:List', status: 1 }],
        roleCode: 'operator',
        status: 1,
      },
    ]);
    ...
    await request(app.getHttpServer())
      .post('/bot-adapter/napcat/webui/session')
      .send({ accountId: '1001' })
  ...
  it('passes heartbeat and revoke ownership evidence to the service', async () => {
    await request(app.getHttpServer())
      .post('/bot-adapter/napcat/webui/session/sess_1/heartbeat')
      .set('user-agent', 'controller-agent')
      .expect(HttpStatus.OK);
    await request(app.getHttpServer())
      .post('/bot-adapter/napcat/webui/session/sess_1/revoke')
      .set('user-agent', 'controller-agent')
      .expect(HttpStatus.OK);
  ...
  it('rejects malformed account and session identifiers before service calls', async () => {
    ...
    await request(app.getHttpServer())
      .post('/bot-adapter/napcat/webui/session/bad%20session/heartbeat')
      .expect(HttpStatus.BAD_REQUEST);
...
describe('NapcatWebuiGatewayService', () => {
  beforeEach(() => {
  ...
  it('creates a browser-safe Gateway session and records sanitized audit', async () => {
    ...
    const service = new NapcatWebuiGatewayService(
      accountService as never,
      containerService as never,
      client as unknown as NapcatWebuiGatewayClient,
      audit,
    );
    ...
    expect(serialized).not.toContain('kt-napcat-1914728559');
  ...
  it('rejects QQ official accounts before looking up any NapCat container', async () => {
    ...
    expect(
      containerService.findPrimaryContainerByAccountId,
  ...
  it('rejects malformed account and session ids before downstream calls', async () => {
    ...
    expect(getThrownMessage(createThrown)).toBe('Bot 账号ID不合法');
...
describe('NapcatWebuiGatewayClient', () => {
  beforeEach(() => {
    requestMock.mockReset();
  });
  ...
  it('posts internal session requests with the secret header and unwraps Gateway data', async () => {
    ...
    const client = new NapcatWebuiGatewayClient(configService as never);
...
describe('NapCat WebUI Gateway audit schema contract', () => {
  it('keeps entity, domain contract, and SQL schema aligned', () => {
    ...
    const botInitSql = readSource('sql/bot-init.sql');
    const tableName = getMetadataArgsStorage().tables.find(
      (table) => table.target === NapcatWebuiGatewayAudit,
    )?.name;

    expect(tableName).toBe('napcat_webui_gateway_audit');
