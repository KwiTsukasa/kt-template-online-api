...
import { toNapcatDockerDeviceOptions } from '../../../../src/modules/bot-adapter/napcat/infrastructure/integration/container/napcat-docker-device-options';
...
} from '../../../../src/modules/bot-adapter/napcat/domain/runtime/napcat-physical-oui-catalog';
  ...
  ({
    get: jest.fn((key: string, defaultValue?: string) => {
      const values: Record<string, string> = {
        NAPCAT_CONTAINER_PREFIX: 'kt-napcat',
        NAPCAT_ROOT: '/vol1/docker/kt-bot-adapter/napcat-instances',
      };
...
describe('NapCat device identity persistence', () => {
  ...
  it('records regression-repair evidence when an existing identity is realigned for QQNT', async () => {
    ...
    repository.seedIdentity({
      accountId: 'account-10001',
      containerId: 'container-first',
      dataDir: '/vol1/docker/kt-bot-adapter/napcat-instances/kt-napcat-10001',
      hostname: 'kt-napcat-10001',
      id: 'identity-1',
      lastLoginEvidence: null,
      macAddress: '02:42:aa:bb:cc:dd',
      machineIdPath:
        '/vol1/docker/kt-bot-adapter/napcat-instances/kt-napcat-10001/machine-id',
    ...
    expect(identity.hostname).not.toBe('kt-napcat-10001');
  ...
  it('applies persisted device options to the NapCat docker create script', async () => {
    ...
    const containerService = new NapcatContainerService(
  ...
  it('reserves create-login containers with a provisional device identity before remote startup', async () => {
    ...
    expect(runtime).toEqual(
      expect.objectContaining({
        dataDir: expect.stringContaining(`kt-napcat-${runtime.id}`),
        id: expect.any(String),
        name: expect.stringContaining(`kt-napcat-${runtime.id}`),
      }),
    );
  ...
  it('adopts the provisional create-login identity when binding the scanned account', async () => {
    ...
    const provisionalIdentity = {
      ...
      containerId: 'container-created',
      dataDir:
        '/vol1/docker/kt-bot-adapter/napcat-instances/kt-napcat-container-created',
      ...
      machineIdPath:
        '/vol1/docker/kt-bot-adapter/napcat-instances/kt-napcat-container-created/machine-id',
      verificationStatus: 'pending',
    } as NapcatDeviceIdentity;
  ...
  it('reuses persisted device identity when rebuilding an existing account container login env', async () => {
    ...
    const recreated = await containerService.ensureRuntimeLoginEnv(
      { id: 'container-1', name: 'kt-napcat-10001' },
      {
