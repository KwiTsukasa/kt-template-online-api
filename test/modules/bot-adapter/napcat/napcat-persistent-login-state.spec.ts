...
describe('NapCat persistent login state contract', () => {
  it('owns the approved third-phase persistence tables', () => {
    const persistenceRoot = join(
      repoRoot,
      'src/modules/bot-adapter/napcat/infrastructure/persistence',
    );
    const sqlSchema = readSource('sql/refactor-v3/00-full-schema.sql');
    const persistenceFiles = existsSync(persistenceRoot)
      ? readSource(
          'src/modules/bot-adapter/napcat/infrastructure/persistence/napcat-account-binding.entity.ts',
        ) +
        '\n' +
        readSource(
          'src/modules/bot-adapter/napcat/infrastructure/persistence/napcat-container.entity.ts',
        )
      : '';
  ...
  it('keeps login truth in persistence rather than in-memory scan sessions', () => {
    const loginSource = readSource(
      'src/modules/bot-adapter/napcat/application/login/napcat-login.service.ts',
    );
  ...
  it('stores challenge and cleanup session ids as uuid-safe text', () => {
    ...
    const qqbotInitSql = readSource('sql/bot-init.sql');
  ...
  it('marks a cancelled pending scan session completed instead of restoring it as active', async () => {
    ...
    store.set({
      ...
      containerName: 'kt-napcat-pending-delete',
      createdAt: Date.now(),
      expiresAt: Date.now() + 60_000,
  ...
  it('replaces the persisted qrcode snapshot when a later status poll returns a fresher qrcode', async () => {
    ...
    const session = {
      ...
      containerName: 'kt-napcat-refreshing-qrcode',
      createdAt: Date.now(),
      expiresAt: Date.now() + 60_000,
  ...
  it('recovers captcha, new-device, and cleanup blockers after cache miss', async () => {
    ...
    loginSessionRepository.rows.push({
      ...
      sessionPayload: {
        ...
        containerName: 'kt-napcat-captcha',
        createdAt: Date.now(),
        expiresAt: Date.now() + 60_000,
        id: 'captcha-recover',
        mode: 'refresh',
    ...
    loginSessionRepository.rows.push({
      ...
      sessionPayload: {
        ...
        containerName: 'kt-napcat-device',
        createdAt: Date.now(),
        expiresAt: Date.now() + 60_000,
    ...
    loginSessionRepository.rows.push({
      ...
      sessionPayload: {
        ...
        containerName: 'kt-napcat-cleanup',
        createdAt: Date.now(),
        expiresAt: Date.now() + 60_000,
        id: 'cleanup-recover',
        mode: 'refresh',
  ...
  it('keeps captcha and new-device challenges recoverable and separate', () => {
    const stateMachine = readSource(
      'src/modules/bot-adapter/napcat/domain/login/napcat-login-state-machine.ts',
    );
