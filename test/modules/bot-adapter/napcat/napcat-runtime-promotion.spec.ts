...
describe('NapCat runtime promotion contract', () => {
  it('exposes Jenkins parameters for runtime image and profile overrides', () => {
    ...
    expect(jenkinsfile).toContain("string(name: 'NAPCAT_IMAGE_OVERRIDE'");
    expect(jenkinsfile).toContain(
      "string(name: 'NAPCAT_DESKTOP_PROFILE_VERSION_OVERRIDE'",
    );
    expect(jenkinsfile).toContain(
      'kubectl ${kubeConfigArg} ${namespaceArg} set env',
    );
    expect(jenkinsfile).toContain(
      'NAPCAT_IMAGE=${napcatImageOverride}',
    );
    expect(jenkinsfile).toContain(
      'NAPCAT_DESKTOP_PROFILE_VERSION=${napcatProfileOverride}',
    );
  });
