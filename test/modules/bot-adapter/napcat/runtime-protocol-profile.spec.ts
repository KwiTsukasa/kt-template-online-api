...
import { NapcatConfigWriterService } from '../../../../src/modules/bot-adapter/napcat/application/runtime/napcat-config-writer.service';
import { NapcatRuntimeProfileService } from '../../../../src/modules/bot-adapter/napcat/application/runtime/napcat-runtime-profile.service';
import { NapcatRuntimeProfileInspectorService } from '../../../../src/modules/bot-adapter/napcat/application/runtime/napcat-runtime-profile-inspector.service';
import { NapcatRuntimeProfileInspectionScriptService } from '../../../../src/modules/bot-adapter/napcat/infrastructure/integration/container/napcat-runtime-profile-inspection-script.service';
import { NapcatRuntimeController } from '../../../../src/modules/bot-adapter/napcat/contract/napcat-runtime.controller';
...
} from '../../../../src/modules/bot-adapter/napcat';
...
describe('NapCat runtime profile generation', () => {
  it('resolves Chinese Desktop Runtime defaults without C.UTF-8 fallback', () => {
    const service = new NapcatRuntimeProfileService({
      get: jest.fn((key: string, defaultValue?: string) => {
        const values: Record<string, string> = {
          NAPCAT_IMAGE: 'kt-napcat-desktop-cn@sha256:profiledigest',
          NAPCAT_RUNTIME_GID: '1101',
          NAPCAT_RUNTIME_UID: '1101',
          NAPCAT_SHM_SIZE: '512m',
        };
    ...
    const profile = service.resolveRuntimeProfile({
      ...
      dataDir: '/vol1/docker/kt-bot-adapter/napcat-instances/linux-pc-a1b2',
  ...
  it('upserts planned runtime and protocol profiles when a managed container is rebuilt', async () => {
    ...
    const service = new (NapcatRuntimeProfileService as any)(
      {
        get: jest.fn((key: string, defaultValue?: string) => {
          const values: Record<string, string> = {
            NAPCAT_IMAGE: 'kt-napcat-desktop-cn@sha256:profiledigest',
            NAPCAT_PROFILE_VERSION: 'napcat-runtime-profile-v1',
            NAPCAT_PROTOCOL_PROFILE_VERSION: 'napcat-protocol-profile-v1',
          };
          return values[key] || defaultValue || '';
        }),
      },
    ...
    const runtimeProfile = service.resolveRuntimeProfile({
      accountId: 'account-1',
      containerId: 'container-1',
      dataDir: '/vol1/docker/kt-bot-adapter/napcat-instances/pc-a1b2c3d4',
      deviceIdentityId: 'identity-1',
    });
    ...
    const plannedProfiles = {
      ...
      deviceIdentity: {
        ...
        machineInfoPath:
          '/vol1/docker/kt-bot-adapter/napcat-instances/pc-a1b2c3d4/QQ/nt_qq/global/nt_data/msf/machine-info',
  ...
  it('keeps unbound planned profiles insertable without a container conflict key', async () => {
    ...
    const runtimeProfile = service.resolveRuntimeProfile({
      accountId: 'account-1',
      dataDir: '/vol1/docker/kt-bot-adapter/napcat-instances/planned-account-1',
    });
  ...
  it('writes account-level NapCat and OneBot configs with minimal reverse WS only', () => {
    ...
    const result = writer.buildConfigFiles({
      account: '10001',
      reverseWsUrl: 'ws://127.0.0.1:48085/bot-adapter/napcat/onebot/reverse',
...
describe('NapCat runtime profile inspector', () => {
  it('builds a bounded SSH inspection script without exposing secrets', () => {
    ...
    const script = service.buildInspectScript('kt-napcat-10001');
...
describe('NapCat runtime profile HTTP API', () => {
  ...
  const protocolProfileRepository = {
    findOne: jest.fn(async () => ({
      accountId: 'account-1',
      reverseWsUrl: `ws://host/bot-adapter/napcat/onebot/reverse?${redactedKeyA}=KT_TEST_AUTH_VALUE`,
  ...
  beforeAll(async () => {
    const moduleRef = await Test.createTestingModule({
      controllers: [NapcatRuntimeController],
      providers: [
  ...
  it('returns sanitized runtime profile evidence through the local HTTP route', async () => {
    const response = await request(app.getHttpServer())
      .get('/bot-adapter/napcat/runtime/detail')
    ...
    expect(response.body).toMatchObject({
      ...
      data: {
        ...
        protocolProfile: {
          reverseWsUrl: `ws://host/bot-adapter/napcat/onebot/reverse?${redactedKeyA}=[REDACTED]`,
