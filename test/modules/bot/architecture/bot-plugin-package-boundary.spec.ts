...
describe('QQBot plugin package boundary', () => {
  it('does not keep built-in plugin transfer services under plugin-platform', () => {
    expect(
      existsSync(
        join(
          repoRoot,
          'src/modules/plugin-platform/infrastructure/integration/builtins',
        ),
      ),
    ).toBe(false);
  });
  ...
  it('does not hard-code built-in plugin packages in platform application services', () => {
    const applicationFiles = collectTsFiles(
      join(repoRoot, 'src/modules/plugin-platform/application'),
    );
    ...
    const violations = applicationFiles.flatMap((filePath) => {
      ...
      return [
        source.includes('@/modules/plugins/')
...
describe('plugin platform package decoupling', () => {
  const platformRoot = join(repoRoot, 'src/modules/plugin-platform');
