...
import { BOT_MESSAGE_SUBSCRIBER_ENTITIES } from '../../../../src/modules/bot-adapter/message-management/bot-message-subscriber.module';
...
const persistenceContract: ReadonlyArray<{
  ...
  },
  {
    ...
    indexes: [
      {
        columns: ['activeKey'],
        name: 'uk_bot_message_publish_binding_active_key',
        unique: true,
      },
    ],
    table: 'bot_message_publish_binding',
  },
  {
    columns: [
      bigint('id', false, true),
    ...
    indexes: [
      {
        columns: ['activeKey'],
        name: 'uk_bot_message_publish_target_active_key',
        unique: true,
      },
    ],
    table: 'bot_message_publish_target',
  },
  {
    columns: [
      bigint('id', false, true),
    ...
    indexes: [
      {
        ...
        name: 'uk_bot_message_delivery_event_target_template',
        unique: true,
      },
      {
        columns: ['status', 'nextAttemptAt'],
        name: 'idx_bot_message_delivery_dispatch',
        unique: false,
      },
      {
        columns: ['processingLeaseUntil'],
        name: 'idx_bot_message_delivery_lease',
        unique: false,
      },
      {
        columns: ['subscriptionId', 'messageEventId'],
        name: 'idx_bot_message_delivery_history',
        unique: false,
      },
    ],
    table: 'bot_message_delivery',
  },
  {
    columns: [
      bigint('id', false, true),
...
describe('message management persistence contract', () => {
  it('registers generic entities in message management and QQBot entities only in its subscriber module', () => {
    ...
    expect(BOT_MESSAGE_SUBSCRIBER_ENTITIES).toEqual([
      BotMessageDelivery,
