...
import { BotAccountMessagePushService } from '../../../../src/modules/bot-adapter/message-management/bot-account-message-push.service';
...
const EXPECTED_ROUTE_PERMISSIONS: Record<string, string[]> = {
  'DELETE /message-management/subscribers/bot/accounts/:selfId/bindings/:id': [
  ...
  'GET /message-management/subscribers/bot/accounts/:selfId/targets': [
  ...
  'PUT /message-management/subscribers/bot/accounts/:selfId/bindings/:id': [
    'MessageManagement:Push:Update',
    'Bot:Account:MessagePush:Update',
  ],
  'PUT /message-management/subscribers/bot/accounts/:selfId/bindings/:id/enabled':
    ['MessageManagement:Push:Toggle', 'Bot:Account:MessagePush:Toggle'],
...
describe('QQBot message-push management controllers', () => {
  ...
  beforeAll(async () => {
    const moduleRef = await Test.createTestingModule({
      ...
      providers: [
        ...
        { provide: BotAccountMessagePushService, useValue: bindings },
        { provide: BotMessageTargetOptionsService, useValue: targets },
      ],
    })
  ...
  it('uses page wrappers only for subscription and template pages', async () => {
    ...
    const bindingResponse = await request(apiUrl)
      .get('/message-management/subscribers/bot/accounts/12345/bindings')
      .expect(200);
  ...
  it('keeps unavailable target lookup at HTTP 200 with an exact safe shape', async () => {
    const response = await request(apiUrl)
      .get('/message-management/subscribers/bot/accounts/12345/targets')
      .expect(200);
  ...
  it.each(['account_unavailable', 'ddns_not_synced'])(
    ...
    async (code) => {
      ...
      const response = await request(apiUrl)
        .post(
          `/message-management/subscribers/bot/accounts/${SELF_ID}/bindings`,
        )
        .send(bindingBody())
        .expect(409);
  ...
  it('rejects unknown enabled and target fields plus non-string source config', async () => {
    ...
    await request(apiUrl)
      .post(`/message-management/subscribers/bot/accounts/${SELF_ID}/bindings`)
      .send(binding)
      .expect(400);
  });
  ...
  it('accepts opaque OpenID targets only for namespaced official accounts', async () => {
    ...
    await request(apiUrl)
      .post(
        '/message-management/subscribers/bot/accounts/qq-official:1020000000/bindings',
      )
      .send(body)
      .expect(200);
  ...
  it('returns only exact public response fields and recursively excludes secrets', async () => {
    ...
    const responses = await Promise.all([
      ...
      request(apiUrl)
        ...
        .expect(200),
      request(apiUrl)
        .get(`/message-management/subscribers/bot/accounts/${SELF_ID}/bindings`)
        .expect(200),
      request(apiUrl)
        .get(`/message-management/subscribers/bot/accounts/${SELF_ID}/targets`)
        .expect(200),
    ]);
    const [
      subscriberList,
