...
function account(overrides: Partial<BotAccount> = {}): BotAccount {
  return Object.assign(new BotAccount(), {
    enabled: true,
    id: '400',
    isDeleted: false,
    selfId: 'bot-a',
...
function binding(
  overrides: Partial<BotMessagePublishBinding> = {},
): BotMessagePublishBinding {
  return Object.assign(new BotMessagePublishBinding(), {
    accountId: '400',
    enabled: true,
    id: '500',
    isDeleted: false,
...
function target(
  overrides: Partial<BotMessagePublishTarget> = {},
): BotMessagePublishTarget {
  return Object.assign(new BotMessagePublishTarget(), {
    bindingId: '500',
    enabled: true,
    id: '700',
    isDeleted: false,
...
function setup(seed: Partial<Store> = {}, adapter = sourceAdapter()) {
  ...
  const concurrentDeliveries: BotMessageDelivery[] = [];
  ...
  let beforeDeliveryConditionalUpdate:
    | null
    | ((deliveries: BotMessageDelivery[]) => void) = null;
    ...
    const rows = (entity === MessageEvent
      ...
      : entity === MessageSubscription
        ...
        : entity === MessageSubscriptionTemplate
          ...
          : entity === BotMessagePublishBinding
            ? store.bindings
            : entity === BotAccount
              ? store.accounts
              : entity === MessageTemplate
                ? store.templates
                : entity === BotMessagePublishTarget
                  ? store.targets
          ...
          if (!item.id && duplicateRace) {
            if (duplicateRace === 'exact') {
              concurrentDeliveries.push(
                Object.assign(new BotMessageDelivery(), item, {
                  id: 'race',
                }),
              );
            }
  ...
  const messageSubscriber = new BotMessageSubscriberAdapter(
    subscriberRegistry,
    {} as never,
  ...
  return {
    ...
    runBeforeNextDeliveryConditionalUpdate: (
      mutate: (deliveries: BotMessageDelivery[]) => void,
    ) => {
...
describe('SystemMessageFanoutService', () => {
  ...
  it('supersedes only strictly earlier unfinished deliveries for the same subscription', async () => {
    ...
    const makeDelivery = (
      ...
      status: BotMessageDelivery['status'],
    ) =>
  ...
  it('fences earlier-history supersession against authoritative processing and cancelled races', async () => {
    ...
    const normal = Object.assign(new BotMessageDelivery(), {
      attemptCount: 3,
    ...
    const racedProcessing = Object.assign(new BotMessageDelivery(), {
      ...structuredClone(normal),
      id: 'raced-processing',
      publishTargetId: 'processing-target',
      status: 'pending' as const,
    });
    const racedCancelled = Object.assign(new BotMessageDelivery(), {
      ...structuredClone(normal),
      id: 'raced-cancelled',
  ...
  it('does not recreate old A work after committed A-to-B-to-A events return to A', async () => {
    ...
    const newestDelivery = Object.assign(new BotMessageDelivery(), {
      id: 'newest',
    ...
    const unrelated = Object.assign(new BotMessageDelivery(), {
      id: 'unrelated',
    ...
    const unrelatedSourceResource = Object.assign(new BotMessageDelivery(), {
  ...
  it('treats same-timestamp BIGINT id 10 as newer than lexical-trap id 9', async () => {
    ...
    const oldDelivery = Object.assign(new BotMessageDelivery(), {
      id: 'old',
      messageEventId: oldEvent.id,
  ...
  it('accepts exact event-target replay and verifies only an exact duplicate-key race', async () => {
    const existing = Object.assign(new BotMessageDelivery(), {
      id: 'old',
      messageEventId: '200',
      publishTargetId: '700',
      status: 'pending',
  ...
  it('stops a stale owner before supersession or inserts after a newer owner completes', async () => {
    ...
    const prior = Object.assign(new BotMessageDelivery(), {
      id: 'prior',
      messageEventId: '100',
      publishTargetId: 'old-target',
      status: 'pending',
  ...
  it('rolls back supersession and an earlier target insert when a subscription later fails', async () => {
    ...
    const priorFailed = Object.assign(new BotMessageDelivery(), {
      id: 'old-failed',
      messageEventId: '100',
  ...
  it('keeps processing and terminal historical deliveries outside supersession', async () => {
    ...
    const statuses: BotMessageDelivery['status'][] = [
      'processing',
      'success',
      'failed',
      'superseded',
