...
import { runPluginCli } from '../../../scripts/plugin/cli';
import { parsePluginManifest } from '../../../src/modules/plugin-platform/domain/manifest';
...
describe('plugin platform CLI', () => {
  let sandbox: string;

  beforeEach(() => {
    sandbox = path.join(
      workspaceTmpRoot,
      `plugin-cli-${process.pid}-${Date.now()}`,
    );
  ...
  it('registers the package script used by local and CI workflows', () => {
    ...
    expect(packageJson.scripts['plugin']).toBe(
      'ts-node -r tsconfig-paths/register scripts/plugin/cli.ts',
    );
  });

  it('creates, validates, packs, and locally installs a plugin through one validation chain', async () => {
    const created = await runPluginCli(
      ['create', 'demo-plugin'],
    ...
    expect(() =>
      parsePluginManifest(readJson(path.join(pluginRoot, 'plugin.json')), {
        pluginRoot,
      }),
    ).not.toThrow();

    await expect(
      runPluginCli(['validate', pluginRoot], silentCliOptions(sandbox)),
    ...
    const packed = await runPluginCli(
      ['pack', pluginRoot],
      silentCliOptions(sandbox),
    );
    ...
    expect(packed.packagePath).toMatch(
      /demo-plugin-0\.1\.0-[a-f0-9]{12}\.plugin\.json$/,
    );
  ...
  it('refuses hidden files, oversized files, and returns stable install ids', async () => {
    ...
    await expect(
      runPluginCli(['pack', pluginRoot], silentCliOptions(sandbox)),
