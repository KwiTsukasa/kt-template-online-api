...
import { PluginPackagePathPolicyService } from '@/modules/plugin-platform/infrastructure/integration/package/plugin-package-path-policy.service';
import { PluginPackageSourceService } from '@/modules/plugin-platform/infrastructure/integration/package/plugin-package-source.service';

describe('PluginPackageSourceService', () => {
  let tempRoot: string;

  beforeEach(() => {
    tempRoot = mkdtempSync(join(tmpdir(), 'plugin-source-'));
  });
  ...
  it('discovers packages from plugin.json without importing package code', async () => {
    ...
    const source = new PluginPackageSourceService(
      new PluginPackagePathPolicyService([tempRoot]),
    );
  ...
  it('resolves entry files from parser-normalized package paths', async () => {
    ...
    const pathPolicy = new PluginPackagePathPolicyService([tempRoot]);
    const resolveEntryFileSpy = jest.spyOn(pathPolicy, 'resolveEntryFile');
    const source = new PluginPackageSourceService(pathPolicy);
  ...
  it('discovers the default built-in plugin root from production dist output', async () => {
    ...
    const distPluginRoot = join(productionRoot, 'dist', 'modules', 'plugins');
    mkdirSync(distPluginRoot, { recursive: true });

    try {
      process.chdir(productionRoot);
      jest.resetModules();
      const { PluginPackagePathPolicyService: RuntimePathPolicyService } =
        await import('../../../src/modules/plugin-platform/infrastructure/integration/package/plugin-package-path-policy.service');
  ...
  it('resolves persisted source package paths to the current dist controlled root', () => {
    ...
    const source = new PluginPackageSourceService(
      new PluginPackagePathPolicyService([distPluginRoot]),
    );

    expect(
      source.resolveDescriptor('src/modules/plugins/bilibili-card', {
        key: 'bilibili-card',
        name: 'Bilibili Card',
        version: '1.0.0',
        entry: 'src/index.ts',
  ...
  it('does not keep platform-side manifest transfer shims', () => {
    const source = readFileSync(
      join(
        process.cwd(),
        'src/modules/plugin-platform/infrastructure/integration/package/plugin-package-source.service.ts',
      ),
      'utf8',
    );
