...
import { PluginTaskService } from '../../../src/modules/plugin-platform/application/task';
import {
...
describe('Plugin platform task API contract', () => {
  it('exposes task management routes under plugin-platform ownership', () => {
    expect(
      collectControllerRoutes([PluginPlatformTaskController]).map(routeKey),
    ).toEqual(
      expect.arrayContaining([
        'GET /plugin-platform/tasks/page',
        'GET /plugin-platform/tasks/:id',
        'POST /plugin-platform/tasks/:id/enable',
        'POST /plugin-platform/tasks/:id/disable',
        'POST /plugin-platform/tasks/:id/cron',
        'POST /plugin-platform/tasks/:id/run',
        'GET /plugin-platform/tasks/:id/runs',
      ]),
    );
  });
