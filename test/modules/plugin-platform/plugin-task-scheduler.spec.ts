...
} from '../../../src/modules/plugin-platform/application/task';
...
describe('QQBot plugin task scheduler', () => {
  ...
  it('registers cron through BullMQ Job Scheduler with a stable scheduler id', async () => {
    ...
    const scheduler = new PluginTaskSchedulerService(
      createConfigService(),
  ...
  it('executes a task job through the platform worker and stores only safe output keys', async () => {
    ...
    const processor = new PluginTaskWorkerProcessor(
      createConfigService(),
  ...
  return {
    get: (key: string) =>
      ({
        ...
        PLUGIN_TASK_QUEUE_REDIS_PREFIX: 'kt:plugin:plugin-task',
      })[key],
  };
}
