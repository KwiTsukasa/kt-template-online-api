...
import { createPluginFromDescriptor } from '@/modules/plugin-platform/infrastructure/integration/runtime/plugin-worker.thread';

describe('generic plugin worker entry', () => {
  let tempRoot: string;

  beforeEach(() => {
    tempRoot = mkdtempSync(join(tmpdir(), 'plugin-worker-'));
  });
...
function createManifest(key: string, entry: string): PluginManifest {
  return {
    assets: [],
    configSchema: {},
    entry,
