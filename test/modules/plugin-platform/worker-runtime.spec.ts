...
} from '../../../src/modules/plugin-platform/infrastructure/integration/runtime/worker-runtime';
import {
  createPluginBullmqWorkerQueueOptions,
  resolvePluginQueueConnection,
} from '../../../src/modules/plugin-platform/infrastructure/integration/runtime/bullmq-plugin-worker-request.queue';
import type {
  PluginWorkerDriver,
  PluginWorkerRequest,
  PluginWorkerRequestQueue,
  PluginWorkerRuntimeOptions,
} from '../../../src/modules/plugin-platform/infrastructure/integration/runtime/worker-runtime.types';
import { createPluginSdk } from '../../../src/modules/plugin-platform/infrastructure/integration/sdk';

class RecordingDriver implements PluginWorkerDriver {
  disposed = false;
  readonly requests: PluginWorkerRequest[] = [];
...
class RecordingRequestQueue implements PluginWorkerRequestQueue {
  private previous = Promise.resolve();

  constructor(private readonly driver: PluginWorkerDriver) {}
...
class GenerationAwareRequestQueue implements PluginWorkerRequestQueue {
    ...
    const run = this.previous
      ...
      .then(async () => {
        if (generation !== this.generation) {
          ...
          error.name = 'PluginWorkerStaleRequestError';
          throw error;
        }
        try {
...
class ExpiringOnceRequestQueue implements PluginWorkerRequestQueue {
  constructor(private readonly driver: PluginWorkerDriver) {}
  ...
  async request(message: PluginWorkerRequest): Promise<unknown> {
    if (message.operationId === 'op-expire') {
      await this.reset();
      throw new PluginWorkerExpiredRequestError(
...
const createRuntime = (driver = new RecordingDriver()) => {
  const runtime = new PluginWorkerRuntime(new RecordingRequestQueue(driver), {
    defaultTimeoutMs: 50,
...
describe('QQBot plugin worker runtime', () => {
  it('accepts a package descriptor in worker runtime options', () => {
    const options: PluginWorkerRuntimeOptions = {
      defaultTimeoutMs: 5000,
      descriptor: {
        entry: 'src/index.ts',
        entryFile: 'D:/repo/src/modules/plugins/sample/src/index.ts',
        manifest: {
          key: 'sample',
          pluginKey: 'sample',
          name: 'Sample',
        ...
        packageRoot: 'D:/repo/src/modules/plugins/sample',
        pluginKey: 'sample',
      },
      installationId: 'install-1',
      pluginKey: 'sample',
...
describe('QQBot plugin worker queue config', () => {
  it('requires an explicit Redis host for plugin worker queues', () => {
    ...
    expect(() => resolvePluginQueueConnection(configService)).toThrow(
      'Bot 插件队列缺少 Redis 主机配置',
    );
  });

  it('uses safe numeric defaults when optional Redis values are blank', () => {
    const options = createPluginBullmqWorkerQueueOptions(
      createConfigService({
        PLUGIN_QUEUE_REDIS_DB: '',
        PLUGIN_QUEUE_REDIS_HOST: 'redis.local',
