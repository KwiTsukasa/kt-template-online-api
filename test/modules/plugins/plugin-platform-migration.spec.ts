...
const legacyPluginRoot = join(repoRoot, 'src/modules/qqbot/plugins');
...
describe('existing plugin platform migration', () => {
  ...
  it('seeds Bilibili card plugin event metadata in refactor v3 SQL', () => {
    ...
    expect(refactorSeedSql).toContain(`version_id = VALUES(version_id),`);
    expect(refactorVerifySql).toContain(`'seed_plugin_version_bilibili_card'`);
    expect(refactorVerifySql).toContain(
      `'seed_plugin_installation_bilibili_card'`,
    );
    expect(refactorVerifySql).toContain(`JOIN plugin p ON p.id = h.plugin_id`);
  });
