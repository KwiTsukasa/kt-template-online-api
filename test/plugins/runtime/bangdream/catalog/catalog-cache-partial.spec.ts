describe('BangDream catalog cache partial loading', () => {
  it('loads only requested catalog keys and their matching static patches', async () => {
    ...
    jest.resetModules();
    jest.doMock(
      '@/modules/plugins/bangdream/src/infrastructure/integration/bestdori.provider',
      () => ({
    ...
    jest.doMock(
      '@/modules/plugins/bangdream/src/infrastructure/storage/static-patch.provider',
      () => ({
    ...
    jest.doMock(
      '@/modules/plugins/bangdream/src/application/bangdream-logger',
      () => ({
        logger: jest.fn(),
      }),
    );
    ...
    const { BANGDREAM_BESTDORI_API_PATHS } = await import(
      '@/modules/plugins/bangdream/src/domain/common/bangdream-protocol'
    );
