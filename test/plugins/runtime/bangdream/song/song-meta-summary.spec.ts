describe('BangDream song meta rank summary', () => {
  it('matches full meta ranking entries for a single song', async () => {
    ...
    const { Server } =
      await import('@/modules/plugins/bangdream/src/domain/catalog/server.model');
    const songModule =
      await import('@/modules/plugins/bangdream/src/domain/song/song.model');
  ...
  it('caches jacket image decoding within a song instance', async () => {
    ...
    jest.doMock(
      '@/modules/plugins/bangdream/src/domain/song/song-resource.repository',
      () => ({
