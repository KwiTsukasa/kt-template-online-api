...
describe('QqbotOfficialWebhookController', () => {
  ...
  beforeAll(async () => {
    const moduleRef = await Test.createTestingModule({
      controllers: [QqbotOfficialWebhookController],
      providers: [
        {
          provide: QqbotOfficialService,
  ...
  it('preserves the exact JSON bytes and forwards official signature headers', async () => {
    ...
    await request(app.getHttpServer())
      .post(
        '/qqbot/official/webhook/1020000000/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
      )
